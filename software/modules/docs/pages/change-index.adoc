...
`DEFAULT`::
  The default behavior applies to all `ATTRIBUTE` columns that are not `DATE` type, `PREFIX_AND_SUBSTRING` for short values,  `PREFIX_ONLY` for long values, and free-form text.
