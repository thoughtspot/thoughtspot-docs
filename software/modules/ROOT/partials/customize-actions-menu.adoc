...
These custom menu items are visible when you select the *More* icon image:icon-more-10px.png[] on an Answer, or during a search, if you use the search terms that the external application expects to receive.
...
To add custom items to the *More* menu image:icon-more-10px.png[] for Answers, navigate to the Admin Console by clicking on the *Admin* tab from the top navigation bar.
