...
:description: Learn about the fields used to create a Snowflake connection with ThoughtSpot DataFlow.
...
When `TRUE`, Snowflake keeps the session active indefinitely, even if there is no user activity. +
 &bull;
When set to `FALSE` user must sign in after four hours of inactivity. Optional field.
Example:;; TRUE
