...
You can sign in to your ThoughtSpot instance Linux shell using SSH and run the command `sharding_diagnostics.sh` from any of the nodes to get a list of undersharded tables. This outputs the list of tables where sharding need to be increased, sorted with the table that is in most need of sharding at the top.
...
It checks for over- or under-sharded tables on your cluster.
 ...
 .. Sign in to your cluster on the command line.
+
[source,console]
----
