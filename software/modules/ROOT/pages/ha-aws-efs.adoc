= Set up disaster recovery for AWS
:last_updated: 02/02/2021
:linkattrs:
:experimental:
:description: This article explains how to set up disaster recovery for your ThoughtSpot cluster using the AWS Elastic File System (EFS).
:jira: SCAL-200783
...
+
Refer to AWS documentation on link:https://docs.aws.amazon.com/efs/latest/ug/getting-started.html?shortFooter=true[creating an EFS File System^].
...
NOTE: At this time, ThoughtSpot does not support AWS Auto Scaling or deployment of ThoughtSpot In-memory cluster across AWS availability zones or regions. ThoughtSpot Connection cluster is supported across AWS availability zones within the same region.
