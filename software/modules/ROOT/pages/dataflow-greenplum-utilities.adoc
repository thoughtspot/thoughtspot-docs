...
----
. Select the Greenplum Client for RHEL 8.
+
...
IMPORTANT: Make sure the server configuration parameter `gp_external_grant_privileges` is set to *ON* for the user running _gpload_.
Also, make sure that port 9015 is opened on the TS cluster.
