...
| Floating point | `DOUBLE` or `FLOAT` | We recommend that you use `DOUBLE`, because it has a range of 15 significant digits, `1.7E +/- 308`. `FLOAT` has a range of 7 significant digits, `3.4E +/- 38`.
...
| Date or time | `DATE`,  `DATETIME`, `TIMESTAMP`, `TIME` | `DATETIME`, `TIMESTAMP`, and `TIME` are stored at the granularity of seconds. `TIMESTAMP` is identical to `DATETIME`; we include it for syntax compatibility.
|===
