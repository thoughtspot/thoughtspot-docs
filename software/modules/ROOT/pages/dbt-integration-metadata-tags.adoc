...
+
If the column type is `measure`: `count`, `count_distinct`, `min`, `max`, `sum`, `average`, `std_deviation`, or `variance`. If you try to set a value not in this list, ThoughtSpot will return an error.
...
Possible values;; Refer to the xref:data-modeling-patterns.adoc#number-formats[number] and xref:data-modeling-patterns.adoc#date[date] formats specified in xref:data-modeling-patterns.adoc[]. You can only set this value if the column data type is `INT`, `BIGINT`, `DOUBLE`, `DATE`, or `DATETIME`, or ThoughtSpot will return an error.
