...
You do not need a view when you want to do a search on top of another search, commonly known as a subquery.
You can use the `IN` keyword instead.

image::search-example-in-keyword.png[Sample search query: sales department store name store name in (top 10 store name by sales bakery)]
...
+
image::in-search-first-step-2020.png[Sample first level search that will be within the in clause]

. Copy that search, and paste it into your new search as the `in` clause.
+
image::in-search-example-2020.png[Sample search with in clause]
