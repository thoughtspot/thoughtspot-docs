...
* Then, aggregate to the coarser search level of detail by using the outer aggregation function, which is Average. That average gives us (7 + 6 + 11 + 18)/4, which results in the 10.5 value in the preceding table.
...
In the following example, our inner aggregation aggregates to the _account_ and _activity classification_ level of detail.
