...
|===
+
In the preceding table, we can see that *hours per activity classification where hours > 5* shows zero hours for the cases where total hours is less than or equal to 5.
...
Note that these formulas will not work whether or not you supply the outer aggregation (`sum`) in the `hours by activity classification` formula. If you supply the outer aggregation, `hours by activity classification` already aggregates to the level of detail specified by the search.
