...
:experimental:
:description: Learn how to upgrade a cluster to a new release, or patch a cluster to update services and allow for bug fixes.
...
. If `tscli cluster status` or `tscli cluster check` return any failures, contact {support-url} before you proceed with the upgrade.
...
----
+
NOTE: During the upgrade process, some services may temporarily be unavailable. Their status in the `tscli cluster status --tail` command might be `FAILURE`. The service should return to `SUCCESS` as the upgrade continues. If a service continues to fail, contact {support-url}.
