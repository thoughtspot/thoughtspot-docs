...
image::filtered-agg-forms.png[Formula example]
...
Try this syntax using the link:{attachmentsdir}/superstore.csv[superstore] example data.
The first `vs` example compares two segments with a single search token:

image::vs-ex-1.png[Search result for store state vs store region sales]
...
image::vs-ex-2.png[Search result for sales store state vs all yearly last 3 years]

You can also provide multiple `vs` instances:

image::vs-ex-3.png[Search result for sales store state vs all vs yearly last 3 years]

Of course, you can compare across different columns as well:

image::vs-ex-4.png[Search result for sales customer age group customer gender department sales age group vs customer gender]
