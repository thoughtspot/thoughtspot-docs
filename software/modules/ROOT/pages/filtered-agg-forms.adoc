...
|====================
|Function|Examples
a|`sum_if(region=’west’, revenue)` a|Takes a measure and one or more attributes. Returns the average of the measure grouped by the attribute(s).
    group_average (revenue, customer region)
a|`count_if(region =’west’, region)` a|Takes a measure and one or more attributes. Returns the count of the measure grouped by the attribute(s).
    group_count (revenue, customer region)
a|`count_if(revenue > 100, red)` a|Takes a measure and one or more attributes. Returns the count of the measure grouped by the attribute(s).
    group_count (revenue, customer region)
|====================
