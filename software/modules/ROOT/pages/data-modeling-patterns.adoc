...
image::profile-locale.png[Change your preferred language in the profile]
...
. Select it to open the *Specify Currency Type* modal.
+
image::specify_currency_type.png[Specify Currency Type]
...
If you use the *From a column* setup to specify the currency format, ThoughtSpot has the following limitations:

* The currency column is automatically selected in every query that uses its linked column, and you cannot remove it.

* In charts, the numbers in axes, data labels, and tooltips do not append the currency symbol. The currency symbol or ISO code is appended in the table view.
