...
:experimental:
:linkattrs:
:description: A relay host for SMTP traffic routes the alert and notification emails coming from ThoughtSpot through an SMTP email server.

ThoughtSpot uses email to send critical notifications to {support-url}. A relay host for SMTP traffic routes the alert and notification emails coming from ThoughtSpot through an SMTP email server.
...
Set up SMTP rules to send critical email notifications to {support-url}.
...
----
+
If you are on 6.1 rather than 6.1.1, or an earlier version than 6.0.5, contact {support-url} to use a custom port.
...
{support-url} monitors these alerts to ensure your cluster's health.
