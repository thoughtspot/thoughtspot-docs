...
Once the table has been created, you need to populate the table with the appropriate data. The date (first column) is the actual calendar data that will be used to map to the custom date values. The following example shows a few records. In this case, the day and month names have been change to Spanish, the first month is June, and the first day of the week is Monday.
