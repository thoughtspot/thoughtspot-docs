...
This is an alternative installation method to the xref:customer-os-ansible-configure.adoc[online cluster installation] and xref:customer-os-offline-script.adoc[offline cluster installation]. If your organization requires that privilege escalation take place through an external tool that integrates with ansible, follow the steps in this article.
