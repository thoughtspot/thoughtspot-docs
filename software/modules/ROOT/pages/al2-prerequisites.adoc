...
. Refer to xref:configuration-options-aws.adoc[AWS configuration options] for the exact specification for the hosts:
 ** CPU
 ** memory
 ** disks
. Refer to xref:launch-an-instance-aws.adoc[Set up AWS resources for ThoughtSpot] to create and launch your AWS virtual machines.
 ** Ensure that your xref:launch-an-instance-aws.adoc#prerequisites-ami[AMI] is an Amazon Linux 2 image.
...
* *R repository*: for R, ensure the machine is able to reach the `CRAN` repository located at link:c[cran.rstudio.com].
