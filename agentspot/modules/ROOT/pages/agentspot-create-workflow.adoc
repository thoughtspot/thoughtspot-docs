...
When a run finishes, AgentSpot examines it and surfaces a pass/fail verdict for that run, so you can tell at a glance whether the run actually succeeded rather than just completed.
// TODO image: capture the pass/fail verdict shown against a finished run in the run-history panel
