...
* Liveboards filters may return a query constraint error. Liveboards that have filters and that are built on Models with the constraint require that the Date field is included as a filter. This ensures the user has the ability to select the date range from the filter. Once selected, subsequent liveboard filter queries will meet the defined date constraint. This date filter could be marked as Mandatory. Example: Select the value first and then the Liveboard will load.
