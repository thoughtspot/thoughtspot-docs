...
a|
[#and-or]
`and/or` | Returns `true` when both conditions are `true`, or when either condition is `true`, otherwise returns `false`. This feature is in Early Access. | `(1 = 1) and (3 > 2) = true` +
`lastname = 'smith' and state ='texas' or (1 = 5) = true` +
`state = 'california'`
|===
