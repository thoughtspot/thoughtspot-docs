ifndef::pendo-links[]
March 2025 [label label-dep]#10.7.0.cl#
endif::[]
ifdef::pendo-links[]
[month-year-whats-new]#Marchy 2025#
[label label-dep-whats-new]#10.7.0.cl#
endif::[]

ifndef::free-trial-feature[]
NOTE: Features marked as [.badge.badge-update-note]#Beta# are off by default. To enable them, contact {support-url}.
endif::free-trial-feature[]

[#primary-10-7-0-cl]

// Business User

////
ifndef::free-trial-feature[]
ifndef::pendo-links[]
[#10-7-0-cl-feature-tag]
[discrete]
=== Spotter in MS Teams [.badge.badge-early-access]#Early Access#
endif::[]
ifdef::pendo-links[]
[#10-7-0-cl-spotter]
[discrete]
=== Spotter in MS Teams [.badge.badge-early-access-whats-new]#Early Access#
endif::[]

// Naomi – jira: SCAL-240915. docs jira: SCAL-?
// PM: Mohil. may not make it into 10.7

endif::free-trial-feature[]
////

[#10-7-0-cl-parameter]
[discrete]
=== Dynamic parameters in Charts

You can now add parameters as variables to the table header, column names and axis names of a chart. Any references to these parameters in a Liveboard or Answer automatically update to show the current value of the parameter.

// Mark. jira: SCAL-224576. docs jira: SCAL-240479
// PM: Vaibhav

[#10-7-0-cl-bilingual]
[discrete]
=== Bilingual keywords for Japanese

// Naomi. jira: SCAL-233663. docs jira: SCAL-239444
// PM: Aashna. on by default for Japanese, EA for other languages.

ThoughtSpot now supports bilingual keywords for the Japanese locale. When your system language is set as Japanese, you can now enter keywords in English and Japanese while searching.

[#10-7-0-cl-number]
[discrete]
=== Number formatting on Attributes

// Mary. jira: SCAL-237236. docs jira: SCAL-?
// PM: Manan

[#10-7-0-cl-cookie]
[discrete]
=== Support cookieless authentication flow for OAuth

// Mary. jira: SCAL-231977. docs jira: SCAL-?
// PM: Prayansh

// Analyst

ifndef::free-trial-feature[]
ifndef::pendo-links[]
[#10-7-0-cl-cyoc]
[discrete]
=== Create your own chart (CYOC) [.badge.badge-beta]#Beta#
endif::[]
ifdef::pendo-links[]
[#10-7-0-cl-cyoc]
[discrete]
=== Create your own chart (CYOC) [.badge.badge-beta-whats-new]#Beta#
endif::[]

// Mark – jira: SCAL-233945. docs jira: SCAL-?
// PM:

endif::free-trial-feature[]

[#10-7-0-cl-joins]
[discrete]
=== Join cardinality change for global joins

// Mary. jira: SCAL-224198. docs jira: SCAL-225298
// PM: Samridh

[#10-7-0-cl-model]
[discrete]
=== Modeling improvements

// Mark. jira: SCAL-218909. docs jira: SCAL-?
// PM: Samridh

[#10-7-0-cl-query]
[discrete]
=== Query tagging: tracking NLS queries in Query Stats Worksheet

// Naomi. jira: SCAL-234949. docs jira: SCAL-?
// PM: Anant. waiting for more info.

ThoughtSpot now distinguishes between regular query consumption and Spotter queries within the Query Stats Worksheet.

[#10-7-0-cl-react]
[discrete]
=== Move Admin and Data tab to React

// Rani. jira: SCAL-235756. docs jira: SCAL-?
// PM: ?

'''
[#secondary-10-7-0-cl]
[discrete]
=== _Other features and enhancements_

// Data Engineer

[#10-7-0-cl-oauth]
[discrete]
=== OAuth for Denodo using Proof Key for Code Exchange (PKCE)

// Naomi. jira: SCAL-226981. docs jira: SCAL-?
// PM: Prayansh

We now support OAuth with PKCE for Denodo.

ifndef::free-trial-feature[]
ifndef::pendo-links[]
[#10-7-0-cl-embrace]
[discrete]
=== Create Connection UX [.badge.badge-beta]#Beta#
endif::[]
ifdef::pendo-links[]
[#10-7-0-cl-embrace]
[discrete]
=== Create Connection UX [.badge.badge-beta-whats-new]#Beta#
endif::[]

// Naomi – jira: SCAL-212284. docs jira: SCAL-?
// PM: Prayansh. waiting on info.

endif::free-trial-feature[]

[#10-7-0-cl-bridge]
[discrete]
=== Bridge support for SQL Server and Oracle

// Naomi. jira: SCAL-206474. docs jira: SCAL-?
// PM: Prayansh

We now support Bridge connections for SQL Server and Oracle.

//[#10-7-0-cl-fe]
// [discrete]
// === Changes to the Data workspace

// This release introduces the following changes to the *Data workspace*:

// - The *Search assist* tab has been removed from Models and Worksheets.
// - The *Join* tab has been updated for Models.
// - The UI of the Data workspace page is different.


// Mark. jira: SCAL-230891. docs jira: SCAL-238598
// PM: Samridh

// Developer

// IT/Ops Engineer

[#10-7-0-cl-eula]
[discrete]
=== Security EULA

// Mary. jira: SCAL-227896. docs jira: SCAL-?
// PM: Aashica

[#10-7-0-cl-org]
[discrete]
=== Implement per-Org URL for sharing

// Mary. jira: SCAL-230582. docs jira: SCAL-?
// PM: Himanshu

[#10-7-0-cl-sage]
[discrete]
=== Parameterized Connection handling in metadata

// Mary. jira: SCAL-230990. docs jira: SCAL-?
// PM: ?

[#10-7-0-cl-first]
[discrete]
=== First query suggestions in Spotter
// Naomi. jira: SCAL-218226. docs jira: SCAL-?
// PM: Samuel Weick. waiting on info.

////
[#10-7-0-cl-convex]
[discrete]
=== Convex - Agentic Spotter implementation
// Naomi. jira: SCAL-214785. docs jira: SCAL-?
// PM: Alok. none needed
////

[#10-7-0-cl-decimal]
[discrete]
=== Remove decimal for Japanese

// Mary. jira: SCAL-235114. docs jira: SCAL-?
// PM: Vaibhav

[#10-7-0-cl-null]
[discrete]
=== Implement null value override with custom value
// Mary. jira: SCAL-231869. docs jira: SCAL-?
// PM: Vaibhav

ifndef::free-trial-feature[]
[discrete]
=== For the Developer

For new features and enhancements introduced in this release of ThoughtSpot Embedded, see https://developers.thoughtspot.com/docs/?pageid=whats-new[ThoughtSpot Developer Documentation^].
endif::free-trial-feature[]