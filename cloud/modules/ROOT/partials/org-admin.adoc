...
NOTE: When switching between Orgs, ThoughtSpot shows only the cluster admin account used to log in to the current session. Other cluster admin accounts are not visible in non-Primary Orgs, regardless of whether they use local or SSO/SAML authentication.
