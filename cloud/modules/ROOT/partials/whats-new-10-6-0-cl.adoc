...
[discrete]
=== Show only relevant filters and parameters in tabs on Liveboards [.badge.badge-early-access]#Early Access#
endif::[]
ifdef::pendo-links[]
[#10-6-0-cl-irrelevant]
[discrete]
=== Show only relevant filters and parameters in tabs on Liveboards [.badge.badge-early-access-whats-new]#Early Access#
endif::[]
Filters and parameters that are not relevant to a tab are hidden by default when you open and view a Liveboard. You can choose to view all filters and parameters in the tab, including those that are not relevant, so that you can quickly create a new view or download the Liveboard without having to switch tabs and apply filters individually. The Show/Hide filters button is only available when there are hidden filters and parameters. To enable this feature, contact your administrator.
...
// PM: Reshma - Awaiting doc JIRA from Reshma. Clarification as to how this differs from what was in 10.5.0.cl - appears that the switcher is now hidden by default if the user belongs to only one org instead of being configurable by the admin. No change in 10.6.0.cl will go GA later.
////
...
You can now reorder the parameters in Liveboards and Answers. When you reorder the parameters in a Liveboard or Answer, your changes are saved and the new order is reflected when anyone opens the Liveboard or Answer. To enable this feature, contact your administrator.
// Mary. jira: SCAL-138848. docs jira: SCAL-238563
// PM: Arpit - include a .gif. What's the use of reordering parameters, what's the result? check if in Early Access panel.
...
// PM: Manan - awaiting confirmation regarding if this should be GA as it was EA in 10.5.0.cl and if not, what the changes are in 10.6.0.cl. Get Manan to review bullet points for 10.6.
...
You can now use the Spotter Conversations Liveboard to track how users are interacting with Spotter. For example, this Liveboard allows you to identify what types of queries users are asking, whether the dataset is able to answer the question, what types of follow-ups users are performing, and how many users are repeating similar queries after receiving an answer.
...
// PM: Damian. confirm user privileges to use the feature.

Users with edit privileges to Worksheets or Models can now create column name aliases, linked to the locale the user defines in their profile. This allows you to search for column names in the target language, instead of typing the columns in English and the keywords in the locale language.
...
// Mark. jira: SCAL-194821. docs jira: SCAL-?
// PM: Samridh. what's the difference between data model editor and model editor?
...
// PM: Prayansh. clarify if ClickHouse is a database, what it is. Add image.
...
[#10-6-0-cl-user]
[discrete]
=== Users and groups [.badge.badge-early-access-whats-new]#Early Access#
endif::[]
ifdef::pendo-links[]
[#10-6-0-cl-user]
[discrete]
...
Export and import users, groups, roles and privileges programmatically, using ThoughtSpot Modeling Language. Use this feature to migrate from one environment to another. This feature can be used so that role configuration does not need to be repeated across multiple orgs.
...
// PM: Aashica. Is this just GA now?
