...
You can now use Spotter to ask questions in natural language about your data, directly within Slack. Follow-up questions remain within the same thread, preserving context. Pin a Worksheet or Model to a channel to ensure quick access to data from that Worksheet or Model. To enable this feature, contact {support-url}.
...
All ThoughtSpot Enterprise clusters are now enabled with orgs by default. Your environment remains a single-tenant environment until you create an org. You can delete all the orgs you created and use only the Primary org if you want a single-tenant environment with orgs enabled.
...
[#10-6-0-cl-ask]
[discrete]
=== Spotter enhancements for Liveboards and Answers

This release brings the following changes to Spotter:

- If a Liveboard uses at least one Spotter-enabled data source, the *Spotter* button appears.
- If none of the data sources used on a Liveboard have Spotter enabled, the *Explore* button appears with a notification indicating Spotter is not available.
- The user profile setting for Spotter is now removed. Once Spotter is enabled on a ThoughtSpot cluster, it is available to all users who are assigned the Spotter privilege.
- All chart actions for Liveboards and Answers are now available in a Spotter conversation, including Include and Exclude filters, Show underlying data, and chart axis actions, like Group axis, Sort, and Filter. Spotter remembers any changes you make when you continue the conversation.
