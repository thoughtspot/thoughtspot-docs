...
ifndef::pendo-links[]
xref:spotter-worksheet-model#optimize-spotter.adoc[Optimize for Spotter].
endif::[]
ifdef::pendo-links[]
xref:spotter-worksheet-model#optimize-spotter.adoc[Optimize for Spotter,window=_blank].
endif::[]
...
Import and export users, groups, roles, and privileges programmatically, using ThoughtSpot Modeling Language. Use this feature to migrate from one environment to another. This feature can be used so that user, group, and role configurations do not need to be recreated. To enable this feature, contact your administrator.
