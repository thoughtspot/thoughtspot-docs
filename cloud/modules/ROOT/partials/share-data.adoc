...
To share a table, Model, or view from the *Data* tab, follow these steps.

. Select the *Data workspace* tab.
. Select the dataset you want to share by selecting the empty checkbox next to it.
. Select *Share*.
+
...
+
NOTE: The option to share specific columns is only available for tables, not for Models or Views.
...
To share a table, Model, or view from within the dataset, follow these steps.
...
You cannot share individual columns for Models or views.
. <<specify-permissions,Specify permissions>>.
