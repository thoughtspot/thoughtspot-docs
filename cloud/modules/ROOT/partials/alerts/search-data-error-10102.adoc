...
Resolution:: This error is displayed when this month number index is not defined.  To resolve this error, the custom calendar must be updated via the ThoughtSpot custom calendar utility in the Data Management application.
+
To update the custom calendar, navigate to the calendar list in Data utilities, and select the more option menu image:icon-more-10px.png[ellipsis icon]. Select *Update*.
+
[.bordered]
image::custom-calendar-update.png[Select Update]
+
Next, select the method of replacing a custom calendar as *Existing table* and select the table used for the calendar.
+
[.bordered]
image::cc-existing-table.png[Select Existing table]
+
