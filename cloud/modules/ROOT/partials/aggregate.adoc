...
[#median]
`median`
a| Returns the value of the measure from the row that has the 50th percentile value.
...
NOTE: Complex queries such as Fan Traps and Chasm Traps will return a query generation error. For more information, see xref:chasm-trap[Chasm traps], and https://community.thoughtspot.com/customers/s/article/What-is-Attribution-and-Chasm-Traps[What is attribution and Chasm traps^] in ThoughtSpot Community.
| `median (sales)`
...
[#rank]
`rank`
a| Returns the rank for the current row. Identical values receive an identical rank. Takes an aggregate input for the first argument. The second argument specifies the order, `'asc' \| 'desc'`.
...
[#rank_percentile]
`rank_percentile`
a| Returns the percentile rank for the current row. Identical values are assigned an identical percentile rank. Takes an aggregate input for the first argument. The second argument specifies the order, `'asc' \| 'desc'`.
