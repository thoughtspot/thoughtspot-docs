...
The following charts, tables, and panels are available on the *TS Stats: System Information and Usage Pinboard*: xref:cluster-summary[cluster summary], xref:critical-alerts[critical alerts], xref:mau[monthly active users], xref:monthly-searches[monthly ad-hoc searches], xref:monthly-pb-views[monthly Pinboard views], xref:top-users[top users last month], xref:top-pinboards[top Pinboards last month], xref:search-users[ad-hoc search by user], xref:users-today[today's users], and xref:configuration-events[configuration events].
