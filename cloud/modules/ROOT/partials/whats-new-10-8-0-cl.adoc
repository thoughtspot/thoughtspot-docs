...
You can now create your own charts in ThoughtSpot using Muze. With the code builder, a developer can create new chart types by writing the code for them. With the visual interface, users who are not developers can configure different chart controls without having to write any code. To enable this feature, contact {support-url}. For more information, see xref:chart-create.adoc[Create charts].
