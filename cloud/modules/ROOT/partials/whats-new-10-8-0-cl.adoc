...
You can now use Spotter to conversationally search your data in Slack. Follow-up questions remain within the same thread, preserving context. Pin a Worksheet or Model to a channel to ensure quick access to data from that Worksheet or Model. To enable this feature, contact your administrator.
...
ifdef::pendo-links[]
xref:spotter-best.adoc[Spotter best practices,window=_blank], xref:spotter-change-analysis.adoc[Spotter change analysis,window=_blank], and xref:spotter-deep-analysis.adoc[Spotter deep analysis, window=_blank].
endif::[]
...
You can now create your own charts in ThoughtSpot using Muze. With the code builder, a developer can create new chart types by writing the code for them. Other users can then use these charts to visualize their data. To enable this feature, contact {support-url}.
