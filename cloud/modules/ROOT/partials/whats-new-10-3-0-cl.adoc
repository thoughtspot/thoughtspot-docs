...
You can now define mandatory date column filters that must be used when running a query. An administrator can define this to prevent users from executing queries without a date range, which could have both performance and cost implications on the underlying data platform. For more information, see
ifndef::pendo-links[]
xref:tml-models.adoc#mandatory[TML for Models] and xref:tml-worksheets.adoc#mandatory[TML for Worksheets].
endif::[]
ifdef::pendo-links[]
xref:tml-models.adoc#mandatory[TML for Models,window=_blank] and xref:tml-worksheets.adoc#mandatory[TML for Worksheets,window=_blank].
endif::[]
