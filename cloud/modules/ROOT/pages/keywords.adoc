= Keyword reference
:page-last_updated: 8/26/2025
:linkattrs:
:experimental:
:page-aliases: /reference/keywords.adoc
:description: Use keywords to help define a search.
:jira: SCAL-192414, SCAL-155983, SCAL-240257, SCAL-310739


Use keywords when asking a question to help define your search.
This reference lists the various keywords.

== Keywords in other languages

Currently, we offer the following keyword translations.

////
| [日本語]({{ site.baseurl }}/reference/keywords-ja-JP.html) | [Deutsch]({{ site.baseurl }}/reference/keywords-de-DE.html) |

 | **[日本語]({{ site.baseurl }}/reference/keywords-ja-JP.html)** |   | **[中文 (简体)]({{ site.baseurl }}/reference/keywords-translate/keywords-zh-CN.html)** | **[Deutsch]({{ site.baseurl }}/reference/keywords-de-DE.html)** | **[Español (latín)]({{ site.baseurl }}/reference/keywords-es-US.html)** | **[Français (Canada)]({{ site.baseurl }}/reference/keywords-fr-CA.html)** | **[Français (France)]({{ site.baseurl }}/reference/keywords-fr-FR.html)** | **[Português (Brasil)]({{ site.baseurl }}/reference/keywords-pt-BR.html)** |
////


- xref:keywords-da-DK.adoc[Dansk]
- xref:keywords-de-DE.adoc[Deutsch]
- xref:keywords-de-CH.adoc[Deutsch (Switzerland): German keyword reference]
- xref:keywords-en-US.adoc[]
- xref:keywords-es-ES.adoc[Español (España)]
- xref:keywords-es-US.adoc[Español (Latinoamérica)]
- xref:keywords-es-MX.adoc[Español (Mexico): Spanish keyword reference]
- xref:keywords-fr-CA.adoc[Français (Canada)]
- xref:keywords-fr-FR.adoc[Français (France)]
- xref:keywords-it-IT.adoc[Italiano]
- xref:keywords-nl-NL.adoc[Nederland]
- xref:keywords-nb-NO.adoc[Norsk]
- xref:keywords-pt-PT.adoc[Português (Portugal)]
- xref:keywords-pt-BR.adoc[Português (Brasil)]
- xref:keywords-ru-RU.adoc[русский (ограниченный выпуск)]
- xref:keywords-fi-FI.adoc[Suomi]
- xref:keywords-sv-SE.adoc[Svenska]
- xref:keywords-zh-CN.adoc[中文 (简体)]
- xref:keywords-zh-HANT.adoc[中文 (繁體)]
- xref:keywords-ja-JP.adoc[日本語]
- xref:keywords-ko-KR.adoc[한국어]

To control language, date, and number formats on the ThoughtSpot UI, see how to set xref:user-profile.adoc[locale preferences in your user profile].

You can only use one set of keywords at a time. For example, if you set your locale to Deutsch, you can only use Deutsch keywords. You can't also use English keywords.

=== General

include::partial$keywords-general.adoc[]

=== Date

include::partial$keywords-date.adoc[]

=== Time
[#time-zone-keywords]
==== Timezone-aware keywords [.badge.badge-early-access]#Early Access#

Time zone-aware keywords ensure that relative date filters in ThoughtSpot — like "today," "yesterday," and "last 7 days" — always reflect the correct time for each user, no matter where they are in the world. This ensures that all searches, Liveboards, and dashboards automatically show time-based data that matches local business hours. For more information, see xref:keywords-time-zone-aware.adoc[].

include::partial$keywords-time.adoc[]

[#text]
=== Text

include::partial$keywords-text.adoc[]

=== Number

include::partial$keywords-number.adoc[]

=== Comparative

include::partial$keywords-comparative.adoc[]

=== Location

include::partial$keywords-location.adoc[]

=== Period

include::partial$keywords-period.adoc[]

=== In / Not in

include::partial$keywords-in.adoc[]

////
=== Help

include::partial$keywords-help.adoc[]
////

'''
> **image:tsu-logo.png[ThoughtSpot Training,role=light-only,width=400]image:tsu-logo-dark.png[ThoughtSpot Training,role=dark-only,width=400]**
>
> * We highly recommend that you register for the free Advanced Keywords course in ThoughtSpot University that covers the information in this article, and watch the following lessons:
> ** https://training.thoughtspot.com/advanced-keyword-searches/1781587[Growth of Keyword^]
> ** https://training.thoughtspot.com/advanced-keyword-searches/1781588[Versus Keyword^]
> ** https://training.thoughtspot.com/advanced-keyword-searches/1781589[Of and Percentage Of Keywords^]
> ** https://training.thoughtspot.com/advanced-keyword-searches/1781647[In and Not In Keywords^]
> * See other training resources at https://training.thoughtspot.com/[ThoughtSpot University^].

'''
