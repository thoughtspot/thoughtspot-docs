...
=== Input table
A logical table created in your CDW and registered as a data object in ThoughtSpot. It combines read-only attribute columns from your existing data model with one or more editable input columns.
...
. In the left-hand data panel, browse or search for *Attribute* columns.
...
- *Column Hierarchy*: Classify as Measure or Attribute.
+
...
After creation, your input table appears in the left-hand data panel on the Search Data page under a dedicated *Input Tables* section. Each table entry can be expanded to reveal its columns, organized by Measure and Attribute hierarchy.
...
Users must be added to the  administrator group to create an input table in the CDW.
...
* Input tables cannot be created using a Date column as a key for this version.
* You cannot create input tables from View, SQL View, Tables and Imported Data as source. You can only create Input Tables from Models as Source.
