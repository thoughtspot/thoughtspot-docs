...
We count the indexterm2:[row]s of data that can be searched. ThoughtSpot accesses data though connections from external data sources, searchable in TS. Data moves into ThoughtSpot through a connection to an external Cloud data warehouse, or through (Free Trial only) CSV upload. Your configuration may label the same data multiple times under different table names, and apply different security rules for different users. We calculate the total row count across all these tables. In ThoughtSpot, we treat views as tables.
