...
:jira: SCAL-136661, SCAL-203358, SCAL-214983, SCAL-230240, SCAL-248988, SCAL-262244

[.lead]
Learn how to connect ThoughtSpot Cloud directly to a {connection} data source using supported authentication methods like OAuth or Personal Access Tokens.
...
. Enter the connection details for your {connection} data source using *Personal Access Token*, *Service Account*, *OAuth*, or *OAuth with PKCE*.
+
[.bordered]
image::embrace-databricks-connection-details-service-acct.png[Connection details]
+
--
For Personal Access token, do the following:

- Enter Host, HTTP Path, Personal Access Token, and Catalog (optional).

For OAuth authentication, do the following:

- Enter Host, HTTP Path, OAuth Client ID, OAuth Client Secret, Scope, Auth URL, Access token URL, and Catalog (optional).

For OAuth authentication with Service Principal, do the following:

- Enter Host, HTTP Path, Service Principal UUID, and Service Principal OAuth Secret.
...
For OAuth Client Credentials with x509, do the following:

- Enter Host, HTTP Path, OAuth Client ID, Private Key, Passphrase (optional), Certificate, Access token URL, and Scope.
...
.. For OAuth with Service Principal connections, enter the following key-value pair:
* Key: catalog
* Value: databricks catalog_name
...
Processes::
Choose this if you want to use this configuration for search indexing, row count stats, or so that scheduled Liveboard queries can use a separate warehouse, credentials, or non-OAuth authentication method instead of the parent connection's defaults.
* Check *Sage indexing* if you want to use this connection for search indexing.
* Check *Row count stats* if you want to use this connection to track row count stats.
* Check *Scheduled Liveboards* if you want to user this connection so that scheduled Liveboard queries can use a separate warehouse, credentials, or non-OAuth authentication method instead of the parent connection's defaults.
. Click *Continue*.
+
...
You can edit additional configurations after you create them, including disabling and deleting them. For details, see xref:connections-databricks-edit.adoc#edit-additional-connections[Edit additional connections of your {connection} connection].
endif::[]
