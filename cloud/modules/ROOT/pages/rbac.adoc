...
[NOTE]
====
* ThoughtSpot Cloud (legacy) users will have RBAC enabled by default ThoughtSpot Cloud 26.10.0.cl onwards.
* ThoughtSpot Cloud (modern) or VPT (Virtual Private ThoughtSpot) users will have RBAC enabled by default ThoughtSpot Cloud 26.11.0.cl onwards.
====
...
Can use Spotter|Allows access to ThoughtSpot Spotter features.|

Can manage catalog|Allows users to create, edit, and manage a data connection to Alation, and import metadata.|
...
|Can download visuals|Allows users to download visual downloads (PDFs of the Liveboard or PNG files), including images from Spotter Answer cards.

|Can download detailed data|Allows users to download data exports (XLSX, CSV, or PDFs of table Answers), including CSV files from Spotter. Spotter PDF reports also require *Can download visuals*.

|===

NOTE: Users who previously did not have download privileges will not be assigned the new download privileges. Administrators can assign them manually. Users who previously had download privileges will automatically be assigned both new privileges.

NOTE: If a user without download privileges previously created a schedule, and they are not granted the new download privileges, the schedule will be paused automatically.
...
* Report and schedule public APIs. For more information, see https://developers.thoughtspot.com/docs/?pageid=whats-new[ThoughtSpot Developer Documentation^].
...
> ** User & Group Management course: Watch the https://training.thoughtspot.com/user-group-management/2499231[Group and Role Management^] lesson.
> ** Permissions course: Watch the https://training.thoughtspot.com/admin-permissions/2499240[What is RBAC?^] lesson.
> ** Multi-Tenancy with Orgs course: Watch the https://training.thoughtspot.com/admin-logical-multi-tenancy-with-orgs/2499359[RBAC with Orgs^] lesson.
