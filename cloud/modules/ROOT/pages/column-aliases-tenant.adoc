...
:jira: SCAL-273076, SCAL-281782, SCAL-294851
...
If you want to set a value that resolves the RLS rule to false, set the column value as TS_WILDCARD_NONE.

An example of an equal rule:

`department =ts_vars(vDepartment)`

if the variable value is:
`vDepartment: TS_WILDCARD_ALL`

then the resulting SQL is:
`TRUE`

if the variable value is:
`vDepartment: TS_WILDCARD_NONE`

then the resulting SQL is:
`FALSE`

An example of a not equal rule:
