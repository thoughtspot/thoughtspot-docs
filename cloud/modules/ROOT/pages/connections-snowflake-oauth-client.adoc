= Configure OAuth with Client Credentials for a {connection} connection
:page-last_updated: 10/01/2025
:experimental:
:linkattrs:

:page-partial:
:connection: Snowflake
:description: ThoughtSpot supports Snowflake’s OAuth with client credentials for a Snowflake connection.
:jira: SCAL-160062, SCAL-180837, SCAL-199788, SCAL-270983, SCAL-267256




ThoughtSpot supports {connection}'s OAuth with Client Credentials for a {connection} connection.
...
//For OAuth, we recommend checking the {connection} link:https://docs.snowflake.com/en/user-guide/oauth-ext-overview[documentation^] to confirm any IDP support and their details.
For more information on creating external OAuth tokens, see link:https://community.snowflake.com/s/article/Create-External-OAuth-Token-Using-Azure-AD-For-The-OAuth-Client-Itself[the Snowflake documentation]. This article documents only the most frequently set-up IDP.
...
== Configuring a {connection} database for OAuth with Client Credentials


To add a {connection} connection to ThoughtSpot using OAuth Client Credentials, you must set up the OAuth integration in {connection}.
...
Note that the `authenticationType` parameter is only included in the JSON payload:
...
== Sharing a Model built from tables in a {connection} connection that uses OAuth


When you share a ThoughtSpot object (Model, Liveboard, or Answer) that references a {connection} connection that uses OAuth authentication, the user is prompted to sign in to {connection} before they can view any data.
When viewing the Liveboard, Answer, or running a search on the Model, ThoughtSpot displays a message telling the user to sign in with a button to initiate the login process.


IMPORTANT: In order for a user to access your shared Model, they must have a default role assigned to their user in {connection}.
