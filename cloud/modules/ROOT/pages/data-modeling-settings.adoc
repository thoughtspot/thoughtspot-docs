...
If you are using ThoughtSpot's xref:dbt-integration.adoc[dbt sync] to create ThoughtSpot Models based on your dbt models, change the data modeling settings in your dbt `schema.yml` file, using xref:dbt-integration-metadata-tags.adoc[ThoughtSpot's dbt metadata tags].
...
| Determines whether data users do not have access to can appear in typeahead suggestions and column knowledge cards.
...
As a best practice, make any data modeling settings in the table when you will be creating multiple Models that use that table.
This way, you won't have to make the same settings in each Model.
The settings will be inherited when you create Models that uses columns from the table.
...
Note that if you make your settings at the table level, and then create a Model that uses columns from the table, the settings are inherited from the table at the point in time that the Model is created.
If you then go back and change the settings at the table level, your changes will not be reflected in the Model.

If you want the Model to have the changes you made at the table level, you must drop those columns from the Model and re-add them.
Then save the Model.
At this point, the new settings will be used in the Model.
Note that any saved Answers or Liveboards based on the Model may display differently because of your changes.
...
To take advantage of ThoughtSpot's aggregate-aware query execution, create aggregate Models that have the same column names as defined in the primary Model. Ensure that the resulting formulas are defined to return accurate results. In the primary Model’s TML, add the association definition. Query switching is based on column names. Therefore, these must match between the primary Model and the associated Models.
