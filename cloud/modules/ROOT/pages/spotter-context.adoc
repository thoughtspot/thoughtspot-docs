...
Use the context feature to add clear, natural language explanations to reference questions. This additional context helps Spotter understand the intent and logic behind a specific answer, reducing ambiguity and improving Spotter’s ability to generalize the learning from a reference question to similar future queries.

[%collapsible]
.See *Natural language context* in action.
====
--
+++
<div class="border">
...
. *Ask a question in Spotter*: Start by asking a question within Spotter and adding the question to coaching. For example, "Show me the most active users".
...
+
Navigate to the *Reference question* tab in the Data workspace and scroll to the question, clicking the *More options* icon and selecting *Fix the answer*.
...
+
[.bordered]
image::ref-ques-correct.png[The Review AI-generated Answers page, displaying "top 10 User GUID User Wala Name Conversations sort by Conversations descending" tokens.]
...
Example;; Imagine a retail data model listing user transactions. These transactions have different statuses like 'Placed', 'Shipped', 'Fulfilled', and 'Returned'. A user might ask, "show me our sales for last month."
+
A simple sum of sales might incorrectly include returned or unshipped orders. The final expected response is `[sales] [fulfillment date=last month][ Order_Status = 'Fulfilled']`. The implicit business rule is that "sales" should only be calculated on completed/fulfilled transactions.
+
You can correct the answer by adding a filter `Order_Status = 'Fulfilled'`. In the natural language context box, you would explain this rule: "When calculating sales or revenue, only include orders with a status of 'Fulfilled’ and refer to ‘Fulfillment date’ unless the user specifically requests a different status (for example, 'Shipped').”
...
* *Maintain coherency*: The information provided in the natural language context should not conflict with other coaching examples, standalone instructions, or default system behaviors. Contradictory information can confuse Spotter and degrade performance.
...
* *Iterate and refine*: Coaching is an iterative process. We recommend starting with a small set of coaching examples and gradually refining them based on the system's performance to avoid overcoaching.
