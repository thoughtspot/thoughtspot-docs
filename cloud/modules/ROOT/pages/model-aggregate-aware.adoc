...
To enable this feature, contact link:https://community.thoughtspot.com/customers/s/contactsupport[ThoughtSpot support].
...
* *Reduce cloud spend and resource consumption* — Minimise the number of rows scanned and reduce the required size or capacity of the underlying data warehouse for high-volume queries.
...
* *Ensure transparent usability* — Enable automatic query switching across all major ThoughtSpot consumption surfaces with no visible change to the user experience.

'''

== How it works

Aggregate awareness is achieved through ThoughtSpot's proprietary tokenised search. When a user submits a query:
...
. If the query cannot be answered by an aggregate Model — for example, the user drills down to a transactional level — ThoughtSpot falls back to the detailed Model automatically.

This allows the user to follow their train of thought from aggregate-level analysis to detailed or vice versa. The analytical path is not constrained.
...
| _"Total revenue by month for the last 7 years"_
| Sales Monthly Aggregate
| ~86 rows

| _"Revenue by department and product category"_
| Sales Company Aggregate
| 10s–100s of millions

| _"Show me individual order line items for last week"_
| Sales (detailed)
| 100s of millions
|===
...
=== Step 1 — Create the detailed Model
...
=== Step 2 — Create aggregate Models
...
* Have the same semantic rules applied as the primary Model (see <<considerations>>).

[NOTE]
====
Aggregate Models do *not* need to be defined in the same connection as the primary Model. This allows you to size compute independently — for example, the detailed Model may require an XXL warehouse while the aggregate Models only need a small instance.
====

=== Step 3 — Define the association in TML

In the primary Model's TML file, add the `aggregated_models` block to declare which aggregate Models are associated and at what date granularity each operates.
...
| _"Revenue by month last year"_
| Sales Monthly Aggregate

| _"Total sales by year"_
| Sales Yearly Aggregate

| _"Revenue by region"_ (no date dimension)
| Sales Company Aggregate

| _"Order line items for last week"_
...
The associated Models will not have the same logical table structure as the primary Model. The primary Model may be a complex star or chasm trap schema, whereas the associated aggregate Models are typically single logical tables (materialised views or aggregate tables in the database).

=== Connection flexibility

Aggregate Models do *not* have to be defined in the same connection as the primary Model. This enables:

* *Independent compute sizing* — The detailed Model may require XXL compute; aggregate Models may only need small instances, reducing cost.
...
* *Formula correctness* — Formulas must be defined at the correct grain for the aggregate. A `SUM` that is additive at the line-item level may need to be treated differently at a pre-aggregated level.
...
* link:https://docs.thoughtspot.com/cloud/26.6.0.cl/tml-models[TML for Models]
* link:https://docs.thoughtspot.com/cloud/26.6.0.cl/scriptability[Import and export TML files]
* link:https://docs.thoughtspot.com/cloud/26.6.0.cl/tml-models#mandatory-models-date-column-filters[Mandatory date column filters in Models]
