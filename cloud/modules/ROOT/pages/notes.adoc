...
`spotter_config` and `is_spotter_enabled` TML properties:: The `sage_config` and `is_sage_enabled` properties in Model and table TML files have been renamed to `spotter_config` and `is_spotter_enabled`. Note that instances where Spotter is disabled retain the previous property names.
