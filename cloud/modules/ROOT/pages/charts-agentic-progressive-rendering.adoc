...
Progressive rendering is available for agentic charts in Spotter, starting with ThoughtSpot 26.10.0.cl.
// SOURCE: SCAL-330485 - "Target version: 26.10.0.cl"; SCAL-330486

By default, the feature is turned off.
Your ThoughtSpot administrator can enable it using the `progressiveAnswerEnabled` cluster-level flag through tscli.
...
When the flag is off, agentic charts continue to use the existing synchronous rendering flow.
