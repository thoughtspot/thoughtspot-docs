...
:jira: SCAL-258586, SCAL-296570, SCAL-313160, SCAL-320900
...
- For RBAC enabled clusters, it requires the *Can Download Visuals* privilege.
...
+
[NOTE]
By default, Single Sign-On (SSO) is available when logging in through a web browser. To enable SSO for the Windows or Mac desktop apps, please contact ThoughtSpot Support.
+
...
ThoughtSpot Connected Slides also supports Personalized views.
...
== Enhanced visualizations [.badge.badge-early-access]#Early Access#
ThoughtSpot automatically optimizes the resolution and scaling of chart PNG exports in the Google Slides add-in, improving their legibility.
For KPI without time-series chart type, the hero number is now automatically sized and positioned to fill the visualization
area, so you no longer need to crop images after inserting or refreshing them.

[.bordered]
image::enhanced-viz-plugin.png[Enhanced visualizations]

These improvements require the *Enhanced Answer Report API* feature to be
enabled on your ThoughtSpot instance. Contact your ThoughtSpot administrator to
enable this feature. If the feature is disabled, the previous default resolution and
layout behavior applies.

When the feature is toggled on or off, existing PNG images embedded in your
slides are updated to the new defaults the next time you refresh the
visualizations.
...
Refresh tokens for the Google Slides add-in have a limit of 24 hours, so if you open the plugin after more than 24 hours, the token expires and the data does not update to show the latest information.