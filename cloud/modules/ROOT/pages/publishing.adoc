...
To enable publishing, you must contact {support-url}.
...
. In the *Data objects* page, select the checkbox for the table you want to parameterize.
. In the *Parameterize Table* window, select a *Field name* and a *Variable*.
. (Optional) Add more field names by clicking *+Add another field* and specifying a variable.
. Click *Parameterize*.

To parameterize a connection, do the following:

. Navigate to the *Data workspace* tab.
. In the left-hand navigation, click *Connections*.
. In the *Parameterize Connection* window, select a *Field name* and a *Variable*.
. (Optional) Add more field names by clicking *+Add another field* and specifying a variable.
. Click *Parameterize*.

You can unparameterize a table or connection later if you need to. For more information, see xref:metadata-objects-unparameterize.adoc[].
...
+
. Optionally, you can choose to parameterize any unparameterized sources by selecting *Parameterize* next to the associated source.
.. Select a *Field* and *Variable* to map a variable to field names to parameterize the table with dynamic values.
.. Click *Parameterize*.
. Click *Publish*.
