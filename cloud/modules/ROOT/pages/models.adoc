...
:model: Model
:jira: SCAL-158357, SCAL-194041 (9.10.5 -drag to create joins), SCAL-194038 (9.10.5 - bulk edit column properties). SCAL-189664 (remove spotapps - removed from image in step 3 of creating a model. SCAL-202208 (sort and filter in columns grid - 9.12.0.cl), SCAL-203404 (model filters limit - 9.12.0.cl), SCAL-221549 (knowledge cards - 10.4.0.cl), SCAL-225747 (models GA), SCAL-250091, SCAL-268921 (Table Alias in model guidelines), SCAL-286673, SCAL-289969, SCAL-303480
...
.Table aliases for chasm trap models [.badge.badge-beta]#Beta#
****
Table aliases for chasm trap models are supported, as well as aliasing the primary fact table. Data security (row-level and column-level security) applies to the aliased table as well as the original table. Progressive filters for table aliases are also supported. To enable this feature, contact {support-url}.
****
