...
NOTE: After you enable the Orgs feature on a cluster, you can’t turn off the ability to create Orgs. However, your environment remains a single-tenant environment until you create an Org. You can also delete all the Orgs you created, and just use the Primary Org, if you would like a single-tenant environment with Orgs enabled.
...
.Enable or Disable Org Switcher display [.badge.badge-early-access]#Early access#
****
You can now enable or disable the display of the org name and *Org switcher* for users. By default, the org name and switcher are visible to all users. Cluster administrators can modify this setting so that users with only a single org do not see the org name and switcher.

////
By default, for all org enabled clusters, the org name and switcher are displayed only for users who are part of multiple orgs. They are hidden for users of a single org.
////
To enable or disable the *Org switcher*, complete the following steps:

. Go to the *Admin settings* tab.
. Select *All Orgs*.
. Under *Application settings*, select *Administration*.
. In *Hide org name* click the *Edit* button.
+
[.bordered]
image::org-name-hide.png[Hide org name]
+
. Toggle the *Status* to *Enable* or *Disable* and click *Save*.
+
[.bordered]
image::org-hide-toggle.png[Hide org name]

If the setting is disabled, all users will start seeing the org name after a refresh of the page.

If the setting is enabled, users in single org environments will stop seeing the org name after a refresh of the page. Users in multiple org environments will continue to see the org name and switcher.
...
>
> * We highly recommend that you register for the free Multi-Tenancy with Orgs course in ThoughtSpot University that covers the information in this article, and watch the https://training.thoughtspot.com/admin-logical-multi-tenancy-with-orgs/2499357[Admin Architecture with Orgs^] lesson.
