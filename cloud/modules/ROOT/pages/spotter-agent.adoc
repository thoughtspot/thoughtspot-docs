...
To start, we will ask Spotter Agent for a quick overview of the data set. Note that Spotter Agent describes the number of columns in the Model, the purpose of the data set, and key use cases. Spotter Agent also suggests columns and example questions for each use case.
...
To use Spotter Agent, you must enable it in your user profile by doing the following:

. Select the user icon to the right of the Help button.
. Select *Profile*.
. In your profile, next to _Experience_, select *Edit*.
. For _Spotter answer experience_, select *Spotter agent*.
. Select *Save*.
+
