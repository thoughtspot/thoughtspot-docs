...
:jira: SCAL-215772, SCAL-183714, SCAL-261681, SCAL-267012, SCAL-330779
...
== Filtering a Liveboard when you have limited access to its Models
When you don't have access to all of the Models used in a Liveboard, there are still a number of capabilities available to you.
...
IMPORTANT: Liveboard filters override filters of the same name in an Answer pinned to a Liveboard.
Example: If a pinned Answer uses a filter called *city*, which is filtered on `chicago` and `seattle` and the Liveboard uses a filter called *city*, which is filtered on `anaheim`, `rochester`, and `winslow`, the Answer-level filter will also display those three cities. If the Liveboard filter is set unfiltered, the Answer-level filter will be unfiltered as well.

=== Limitations
The following limitations apply is column-level security or column security rules are in use.

* If a Liveboard is shared with a user, with at least one filter chip that is restricted using column-level security or column security rules:
** The Liveboard will be locked for that user.
** The Liveboard will not appear in the Liveboard list page for that user.
+
To resolve this, you can either grant the user access to all the columns that are included as filter chips on that Liveboard, or remove filter chips of inaccessible columns.

* If a Liveboard includes a visualisation that does not populate because it uses one or more inaccessible columns, the user cannot interact with the filter chips (even though all the chips are accessible):
** When the chips are opened, they are read only.
...
Note that Liveboard filters only apply to the tables and charts that are based on Models.
If a Liveboard also includes tables and charts that were created from underlying tables, the filters don't apply to them.

To create one filter that filters visualizations based on multiple Models, you can link the Model columns, at the Liveboard level.
...
When you apply a filter, the Liveboard is not automatically saved with your filter applied to every object in the Liveboard that was created from a Model.
...
NOTE: Filters and parameters that are not relevant to a tab are hidden by default when you open and view a Liveboard.


== Linked filters
You can link columns from multiple Models, to create one Liveboard filter for visualizations that come from different Models.
...
. Set the new values. Click *Apply*
. Repeat this individually for all the filters to be edited.

==== Centralized filter window

With this centralized window, you can edit the values for multiple filters and parameters together and then apply all the changes to the Liveboard at once.

. Click any of the filters from the row at the top of the Liveboard to open the centralized filter modal.
+
[.bordered]
image::new-filter-modal.png[Centralized filter modal]

. All your Liveboard filters will be on the left navigation of this centralized modal. Edit multiple filters at once and click *Apply* to see the changes.
+
Click *Reset to View Default* to restore the filter settings saved for this View.

=== Edit mode
If you open the filter in the *Edit mode*, by selecting the *Edit* button at the upper right of the Liveboard, you can edit all the filter attributes like:

* Default values of the filter
* The visualizations that the filter should apply to
* Linked filters across models
* Filter name
* Making it a mandatory filter or otherwise

You can select each filter individually to change any of these attributes, then click *Apply*.
...
> ** https://training.thoughtspot.com/creating-and-editing-liveboards/2539334[Add Liveboard Filters^]
> ** https://training.thoughtspot.com/creating-and-editing-liveboards/2169352[Apply Liveboard Filters^]
