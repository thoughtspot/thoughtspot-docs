...
To enable the Contact and Case objects requires the Customer Service plug-in (com.sn_customerservice), and users require the `csm_ws_integration` role for full API access. External users with `sn_customerservice.customer` or `sn_customerservice.consumer` roles can access the case API endpoints.
