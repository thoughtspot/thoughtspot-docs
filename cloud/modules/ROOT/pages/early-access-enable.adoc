...
Custom sort order on Answer page::
Define a custom sort order for attributes directly from the Answer page. For more information, see xref:answer-custom-sort.adoc[Define or edit a custom sort order for an Answer].
...
Spotter Agent:: You can now ask questions directly about your data, ask for suggestions on the next direction you should take in your analysis, and ask Spotter how it calculated fields like formulas. For more information, see xref:spotter-agent.adoc[]. To enable this feature, navigate to *Admin > ThoughtSpot AI* and select Spotter Agent for *Spotter answer experience*.

Answer Why questions:: Spotter can now answer questions about why a metric has changed, such as "Why did sales drop last month?" For more information, see xref:spotter-why.adoc[]. To enable this feature, navigate to *Admin > ThoughtSpot AI* and set Answer Why questions to *Enabled*.

Natural language instructions:: You can now teach Spotter core concepts, default behaviors, and specific data nuances by adding natural language instructions directly to a data model. For more information, see xref:natural-language-instructions.adoc[]. To enable this feature, navigate to *Admin > ThoughtSpot AI* and set Natural Language instructions to *Enabled*.
