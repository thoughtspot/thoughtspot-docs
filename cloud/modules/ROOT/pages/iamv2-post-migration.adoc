...
After migration, authentication will continue as normal for customers with IAMv1 configured.
...
+
If you have the new Admin portal settings enabled, select *User management* > *Authentication*. The Authentication UI will be like the following:
+
[.bordered]
image::admin-2.png[SAML IAMv2]

. Check the expiry date of your IdP signing certificate.
+
If the certificate has already expired, renew it before you update any of the values that follow. If it has not expired, continue to the next step.
...
. Re-upload the IdP certificate in your ThoughtSpot SSO configuration.
+
Depending on your IdP settings, changes to the Assertion Consumer Service URL and Audience can affect the certificate. Re-uploading it ensures the certificate remains valid and aligned with the new configuration.
. Verify that the SAML attribute mappings in ThoughtSpot are exactly as intended, then save the configuration.
+
...
NOTE: After migrating to IAMv2, usage of space and special characters will not be allowed while creating Orgs. This is to keep Org names DNS compliant. This restriction applies to only new Org names where per Org subdomain is enabled, and does not impact existing Orgs.
