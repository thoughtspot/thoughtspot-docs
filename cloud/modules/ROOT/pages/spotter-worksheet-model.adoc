...
To improve search accuracy it's important to index Worksheet and Model attribute columns, because it allows ThoughtSpot to effectively retrieve and provide sample values associated with those columns. These sample values, combined with the column name, are passed to the LLM, which enables it to do the following:
...
IMPORTANT: You must have table edit access in the Model or Worksheet to use _Optimize for Spotter_.
...
+
NOTE: If a window appears with the message "Some columns were not updated", it will list which columns could not be indexed. To index those columns, contact your administrator to get edit access to the underlying table.
