...
To improve search accuracy it's important to index Worksheet and Model attribute columns. *Optimize for Spotter* makes this a lot easier to do. Indexing is critical because it allows the system to effectively retrieve and provide sample values associated with specific columns in a Worksheet or Model. These sample values, combined with the column name, are passed to the LLM, enabling it to:

* Understand the column's properties better
