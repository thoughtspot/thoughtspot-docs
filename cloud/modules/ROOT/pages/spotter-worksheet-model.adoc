...
One of the following messages appears if it is not possible to optimize the Worksheet or Model:

- If a "Some columns were not updated" appears, it lists which columns could not be indexed. To index those columns, contact your ThoughtSpot administrator to get edit access to the underlying table.

- "Optimize for Spotter" guidance message appears when there are additional steps recommended for Spotter Optimization. In this case, refer to xref:worksheet-readiness.adoc[Worksheet readiness for Sage] for guidelines on how to prepare your Worksheet to improve accuracy.
+
[.bordered]
