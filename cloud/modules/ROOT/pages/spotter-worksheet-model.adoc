...
Prefer adding appropriate column descriptions to columns to help LLMs understand more about that column and its values while generating an Answer:: Add appropriate column descriptions to columns to provide more clarity and help Spotter’s judgement on which column to use to answer a query.
...
Leverage column descriptions to instruct the LLM on how to use or not use that column in answering a question:: Column descriptions tell Spotter how to use the data contained to answer questions.
Example::: A column containing date month values as 'Jan', 'Feb', etc. can have a description like "Represents date months in mmm format. For example, Jan". This added context ensures Spotter is able to map the data values clearly.

Restrict column descriptions to 200 characters in length:: Description will not be truncated in passing to the LLM.
...
xref:worksheet-formula.adoc[Create formulas] or xref:custom-sets.adoc[sets] for high cardinality columns::
