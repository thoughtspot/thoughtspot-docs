...
Getting started with ThoughtSpot Cloud [.badge.badge-ga-relnotes]#GA#::
...
[#upload-custom]
Upload custom geo maps [.badge.badge-ga-relnotes]#GA#::
...
[#parameters]
Parameters [.badge.badge-ga-relnotes]#GA#::
...
[#change-filter]
Change filter order for Liveboard filters [.badge.badge-ga-relnotes]#GA#::
...
[#tml-for]
TML for Monitor alerts [.badge.badge-ga-relnotes]#GA#::
...
[#remove-attached]
Remove attached files from scheduled emails [.badge.badge-ga-relnotes]#GA#:: For data security, you may choose to remove attached files from scheduled Liveboard emails. When this feature is enabled, scheduled Liveboard email recipients will receive an email with a link to the Liveboard, which they will need to sign in to see.To ensure that only signed-in users can access your data, contact {support-url} to enable this option. For more information, see xref:schedule-liveboards.adoc#disable-csv-pdf[Scheduled Liveboards].
[#add-a]
Add a custom message in the alert notification [.badge.badge-ga-relnotes]#GA#:: When creating an alert for a KPI chart, you can now include a custom message in the alert update email. You can use this feature to specify next steps the alert subscribers should take once the KPI reaches a certain threshold.
[#disable-heatmap]
Disable heatmap data labels [.badge.badge-ga-relnotes]#GA#::
...
[#show-timezone]
Show timezone in scheduled Liveboards modal [.badge.badge-ga-relnotes]#GA#:: When scheduling Liveboards, you can now specify your timezone.
[#search-suggestions]
Search suggestions on columns with many values [.badge.badge-ga-relnotes]#GA#::
...
[#parameter-runtime]
Parameter runtime overrides [.badge.badge-ga-relnotes]#GA#::
...
[#scheduled-or]
Scheduled or downloaded Liveboard PDF width [.badge.badge-ga-relnotes]#GA#::

By default, scheduled and downloaded Liveboard PDFs now render at a width of 1920 px. This ensures that most Liveboard PDFs you download or schedule look the same as the Liveboards you see in ThoughtSpot. You can also change this default width to a different width for your company, by contacting {support-url}. See xref:liveboard-download.adoc#pdf-width[Download a Liveboard as a PDF] and xref:liveboard-schedule.adoc#pdf-width[Schedule a Liveboard job].
[#specify-time]
Specify time zone when scheduling Liveboards [.badge.badge-ga-relnotes]#GA#::
...
[#group-agggregate]
Group aggregate enhancements: filters [.badge.badge-ga-relnotes]#GA#:: Group aggregate filter enhancements are GA and on by default. Previously, when using group_aggregate formulas, users could use query_filters to include filters entered in the search bar in the formula's value, but it was difficult to write a formula that accepted filters from only a single column. +
...
[#data-labels]
Data labels on a dark background [.badge.badge-ga-relnotes]#GA#::
...
[#text-keywords]
Text keywords enhancements [.badge.badge-ga-relnotes]#GA#::
...
[#mandatory-liveboard]
Mandatory Liveboard filters [.badge.badge-ga-relnotes]#GA#::
...
[#custom-comparison]
Custom comparison points for KPI charts [.badge.badge-ga-relnotes]#GA#:: You can now set your KPI chart to display a percent change comparison between the most recent data point and a custom comparison point. For example, when searching for `sales monthly`, you can choose to see percent change between this month and the previous month, previous quarter, or previous year. For more information, see
xref:chart-kpi.adoc#kpi-custom-comparison[Custom comparison points].
[#change-analysis]
Change analysis for KPI charts [.badge.badge-ga-relnotes]#GA#::
...
[#thoughtspot-sync]
ThoughtSpot Sync [.badge.badge-ga-relnotes]#GA#::
...
Schedule ThoughtSpot Sync [.badge.badge-ga-relnotes]#GA#:: When scheduling a sync with an external app, you can now choose to schedule every 5, 10, 15, 20, 30, and 45 minutes, in addition to the other existing schedule options.

[#hubspot-sync]
HubSpot Sync mapping fields [.badge.badge-ga-relnotes]#GA#:: When creating a sync to HubSpot, you can now use email as a mapping field on the Contacts object type, as well as ContactID.
[#apply-multiple]
Apply multiple changes to charts at once [.badge.badge-ga-relnotes]#GA#:: The apply multiple changes feature is now GA and on by default. In previous releases, every time a user reordered the fields in the chart configuration panel, the chart reloaded. When creating a visualization with multiple fields, users often need to make multiple rearrangements to reach a desired chart. Now users can make multiple configurations and apply them at the same time rather than waiting in between each configuration. For more information, see
xref:chart-x-axis.adoc[Reorder labels on the axis or legend].
[#specify-format]
Specify format when downloading charts and tables [.badge.badge-ga-relnotes]#GA#::
...
[#mobile-app]
Mobile app Liveboard experience [.badge.badge-ga-relnotes]#GA#:: ThoughtSpot is updating the Liveboard experience for the mobile app. Mobile app users can now view KPI charts in Liveboards.

[#mobile-app-load]
Mobile app Load More button [.badge.badge-ga-relnotes]#GA#:: When using ThoughtSpot Mobile, you can now press the *Load More* button at the bottom of a table to see the next 100 rows of data.
[#improvements-to-french]
Improvements to French keywords [.badge.badge-ga-relnotes]#GA#::
This release improves the translated French keywords. See xref:keywords-fr-FR.adoc[].
[#notification-preferences]
Notification preferences [.badge.badge-ga-relnotes]#GA#::
...
[#sql-server]
SQL Server connection [.badge.badge-ga-relnotes]#GA#:: You can now create connections from ThoughtSpot to SQL Server. For details, see xref:connections-sql-server.adoc[SQL Server].
[#generic-jdbc]
Generic JDBC connection [.badge.badge-ga-relnotes]#GA#:: You can now create connections from ThoughtSpot to Generic JDBC databases. For details, see xref:connections-genericjdbc.adoc[Generic JDBC].
[#certify-amazon]
Certify Amazon Aurora and Amazon Relational Database Service (RDS) for PostgreSQL [.badge.badge-ga-relnotes]#GA#:: You can now connect to and query Amazon Aurora and Amazon RDS for PostgreSQL databases.
[#databricks-catalog]
Databricks Catalog field [.badge.badge-ga-relnotes]#GA#:: When creating a connection to Databricks, you can now enter your catalog name under the *Catalog* field. For more information, see xref:connections-databricks-add.adoc[Add a Databricks connection].
[#create-a-sync]
Create a sync as an admin [.badge.badge-ga-relnotes]#GA#:: When creating a sync to an external app, admin users will no longer see destinations created by other users in the *Destinations* dropdown menu. Admin users will still be able to see a list of all destinations created by users in their organization when they navigate to the *Destinations* tab in the Data workspace.
[#delete-table]
Delete table columns using TML [.badge.badge-ga-relnotes]#GA#::
...
[#edit-join]
Edit join conditions using TML [.badge.badge-ga-relnotes]#GA#::
...
[#private-preview]
Private Preview features [.badge.badge-ga-relnotes]#GA#::
...
[#cross-region]
Cross-Region Disaster Recovery [.badge.badge-ga-relnotes]#GA#::
