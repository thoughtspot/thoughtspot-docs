...
:page-last_updated: 3/9/2023
:linkattrs:
:experimental:
:page-layout: default-cloud
:author: Teresa Killmond
:page-aliases: /admin/ts-cloud/scriptability.adoc
:description: Export and import Models, views, SQL Views, tables, Liveboards, Monitor alerts, and Answers in a human-readable format.



Use ThoughtSpot Modeling Language (TML) to export and import Models, views, tables, Liveboards, Monitor alerts, and Answers in a human-readable format.

ThoughtSpot developed its own scriptable approach for exporting, enhancing, and migrating Models, views, tables, Liveboards, Monitor alerts, and Answers.
...
* *Making bulk changes*, such as mass renaming of objects defined by Models, and managing duplicates
...
Refer to the following tables for required permissions for importing, editing, and exporting the TML for Liveboards, Answers, Models, tables, and views.
...
For example, if you import a Liveboard that is built on a Model that is built on a table, you must have *view* permission for the Model.
When importing a new Model or view, you must have the *can manage data* permission.
| *Can manage data*, if the object or any of its dependents is a Model or view.
| *Edit* permission on the existing object.
The dependents must already exist in the cluster.
You must have *view* permissions for the first-level dependent.
When importing a Model or view, you must have the *can manage data* permission.
...
////
Columns are exported in the order in which they appear in the table's landing page by default. For Models and models, columns are sorted in the order in which they appear on the columns tab of the model editor.
...
1. Navigate to the Liveboard, Answer, View, table, or Model you want to export.
...
[NOTE]
When you export a TML file, the Object ID is included by default. If needed, you can open the TML file and manually remove it.
...
+
[.bordered]
image::edit-model.png[Edit TML - object list page]
+
...
You can overwrite an existing Model, view, table, Answer, or Liveboard, by downloading the `TML` file, making any necessary changes, and then re-uploading the `TML` file.
...
In this case, we are updating a single Model.
...
. Navigate to the *Data workspace* > *Utilities* page.
. Select *Import TML*.
+
////
Here, we are uploading the edited *TCPH WS* Model.

![Start Model update from file]({{ site.baseurl }}/images/scriptability-worksheet-update.png "Start Model update from file")
////
. In the *Import* interface, click *Select .tml or .zip files to upload*. You can also select the applicable method of import:
- .. Create new objects or update existing objects or do both as applicable.
- .. Only update existing objects.
- .. Only create new objects.
+
[.bordered]
image::import-tml.png[Import TML file]
...
To migrate an Answer, Liveboard, view, or Model from one cluster to another, follow these steps.
...
. Navigate to the *Data workspace* > *Utilities* page, and click the *Import TML* button.
+
////
. To upload a Model or view, select the *More* icon image:icon-more-10px.png[more options menu] in the upper-right side of the screen.
Then, select *Import TML*.
+
image::scriptability-cloud-worksheet-view-import.png[Import Model or View TML]
...
With async import TML, users can submit large or complex imports and continue working while the import runs in the background, unlike the previous import TML which required users to wait for the import to finish before moving onto the next task.
The async import has a new UI and can be enabled from the *Data modelling* option under *Application Settings* on the *Admin settings* page. Once enabled, you can access the new UI from the *Data workspace* > *Utilities* > *Open TML utility*.

[.bordered]
image::utilities1.png[The utilities menu]

When a user submits TMLs for import, the system processes them in the background and returns a task ID. Users can use this task ID to check the status of their import at any time, without waiting for the operation to complete in real time. This approach avoids timeout errors, slow imports, supports large or complex imports without having to split it into smaller chunks , and enables users to monitor progress as needed.

NOTE: With Async TML import, the ability to edit and revalidate a failed TML directly in the UI has been removed. Previously, users could fix errors in the TML editor and revalidate a single file. Now with async import, users must edit the TML locally and re-upload it for import and validation. +
Hence, it is recommended to use Async TML import only for large and complex workloads. For the other smaller imports, continue to use the real time import TML functionality which also supports editing in the TML editor.

*Advanced settings* +

To enable these settings, contact {support-url}. Once enabled, it gives users additional control over the import.
Users can choose to:

* Import only when all files pass validation, or
* Skip files with errors and import only the ones that pass.

[.bordered]
image::async-tml-adv.png[The Advanced settings]

[#delete]
== Deleting columns, joins, and RLS rules
You can delete joins at the table level, table RLS rules, and table, Model, View, and SQL View columns. However, keep in mind the following behaviors:
...
There are certain limitations to the changes you can apply by editing a Model, Answer, table, view, or Liveboard through TML.
