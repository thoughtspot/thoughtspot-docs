...
// CHANGED: Added SCAL-190432 to jira attribute
:jira: SCAL-160062, SCAL-245587, SCAL-270983, SCAL-208437, SCAL-190432
...
[NOTE]
====
OAuth requires users to log in and authorize. The frequency of login will depend on the token expiry.

ThoughtSpot supports Authorization Code grant flow in OAuth authentication.
====
...
+
NOTE: Not all configurations will allow you to change the URI from `api://` to `https://`.
+
...
// CHANGED: Expanded the IMPORTANT note to explain all three valid values for
// external_oauth_any_role_mode, not just ENABLE. Previously only mentioned ENABLE.
IMPORTANT: If you use _SESSION:ROLE-ANY_ in scope, you must configure the `external_oauth_any_role_mode` parameter in the security integration.
Set it to `ENABLE` or `ENABLE_FOR_PRIVILEGE` to allow users to switch to roles not defined in the OAuth access token.
The default value is `DISABLE`.
See the parameter reference table below for a description of all supported values.

==== Security integration format

// CHANGED: Moved external_oauth_any_role_mode to its correct position immediately
// before external_oauth_snowflake_user_mapping_attribute, matching Snowflake's
// documented parameter order. Clarified that the value shown is one option;
// see the table below for all valid values.
[source]
----
create security integration external_oauth_azure_2
    ...
    external_oauth_any_role_mode = 'ENABLE'                        -- optional; see values below
    external_oauth_snowflake_user_mapping_attribute = 'login_name';
----

// CHANGED: Added new parameter reference table for external_oauth_any_role_mode.
// This is the primary change requested in SCAL-190432 — the previous doc did not
// explain the three valid values or their behaviour.
[cols="1,3", options="header"]
|===
|Value |Description

|`DISABLE`
|(Default) Users can only assume roles that are explicitly included in the OAuth access token.
Attempts to use any other role are rejected.

|`ENABLE`
|Users can switch to any role assigned to them in {connection}, even if that role is not present in the OAuth access token.

|`ENABLE_FOR_PRIVILEGE`
|Only users who have been granted the `OAUTH_USE_ANY_ROLE` privilege in {connection} can switch to a role not included in the OAuth access token.
All other users are restricted to roles in the token.
|===

NOTE: You can set `external_oauth_any_role_mode` when creating the security integration (`CREATE SECURITY INTEGRATION`) or update it on an existing integration using `ALTER SECURITY INTEGRATION`.
Refer to the https://docs.snowflake.com/en/sql-reference/sql/alter-security-integration-oauth-external[{connection} documentation^] for details.

.Example security integration format
// CHANGED: Same fix applied to the example block — moved external_oauth_any_role_mode
// to the correct position before external_oauth_snowflake_user_mapping_attribute.
[source]
----
create security integration external_oauth_azure_2
    type = external_oauth
    ...
    external_oauth_issuer = '<AZURE_AD_ISSUER>'
    external_oauth_jws_keys_url = '<AZURE_AD_JWS_KEY_ENDPOINT>'
    external_oauth_audience_list = ('urn:spn:<SNOWFLAKE_APPLICATION_ID_URI>')
