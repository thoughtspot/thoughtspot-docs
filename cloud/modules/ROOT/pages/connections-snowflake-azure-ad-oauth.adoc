...
* Validate the administrative information (tokens) from Azure AD.
* Provide the appropriate {connection} data access to users based on their associated user role, based on the received administrative data (the OAuth token).
...
. For Token Name, enter the token name.
...
You get the refresh token only if you requested the `offline_access` scope.
+
...
. Execute the following command to get the administrative data (access token) with password `grant_type`:
+
----
...
. Execute the following command for getting administrative information (access token) with `refresh_token` as `grant_type`:
+
----
 curl -X POST -H "Content-Type: application/x-www-form-urlencoded;charset=UTF-8" \
   --data-urlencode "client_id=<client_id>" \
