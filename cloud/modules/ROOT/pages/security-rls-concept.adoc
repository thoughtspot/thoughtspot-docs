...
:jira: SCAL-176169 - (correction on RLS on Models), SCAL-230531, SCAL-264258
...
So, searches, Answers, Models, and Liveboards that rely on a table's data fall under RLS rules.

CAUTION: RLS is defined at the table level. Therefore, you must define table-level joins before you define RLS.

== Model queries and RLS

You cannot set RLS rules on Models, only on tables.
However, administrators can disable RLS on Models that are derived from tables with RLS rules.
After RLS rules are disabled, users with access to the Model can see all its data.

When RLS is disabled, default Model queries only take into account RLS rules on tables whose columns appear in the query.
Other related tables that may underlie the Model are ignored.
This means that not all RLS rules on underlying tables are applied when a user queries a Model.

When RLS is enabled, ThoughtSpot implements a default strict application of RLS rules, which takes into account RLS rules from all the tables underlying the Model.
This is recommended if you have key dimension tables that Models rely on but that are not necessarily regularly accessed through query. If you do not want to take into account RLS rules from underlying tables, contact {support-url}. Additionally, ThoughtSpot's RLS logic may override the table join types. If you would like your RLS implementation to always take into account the table join types, contact {support-url}.
...
* They can also disable RLS rules on individual Models.
...
NOTE: When Bypass RLS is checked on a Model, it applies to queries involving tables, but NOT to queries on Views based upon those tables.
...
+
When you share, share Models.
This is a general best practice.
Models simplify the data environment for end users;
they only need to choose among a few sources, rather than many tables.
Also, one Model can also combine data from several tables.
...
> * For information on bypassing rules on a Model, see xref:model-inclusion.adoc#[Change inclusion, join, or RLS for a Model].
