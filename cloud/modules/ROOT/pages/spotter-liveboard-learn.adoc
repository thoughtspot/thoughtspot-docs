...
:page-aliases:
:description:
:jira: SCAL-300551, SCAL-289467

Learning from Liveboard lets you point Spotter at a trusted Liveboard — one that already reflects how your team queries a dataset — and generate memory from it. Spotter analyzes the charts and their underlying queries, extracts xref:spotter-memory.adoc#rule-recipe[rules and recipes], and uses them as context when answering future questions on the associated data models.
...
Learning from Liveboards is off by default and is enabled by the same flag as xref:spotter-conversation-learn.adoc[learning from conversations].
...
If a Liveboard touches Models the user cannot access, those Models are excluded — memory generation proceeds on the accessible models only.
...
. The *Define context* modal appears. Optionally add context to help guide generation. All accessible data models associated with the Liveboard are selected by default; deselect any you don't want to generate memory for.
+
[.bordered]
...
* Ask Spotter. In a conversation, ask "What do you remember about this dataset?" and Spotter will surface relevant rules it is holding.
