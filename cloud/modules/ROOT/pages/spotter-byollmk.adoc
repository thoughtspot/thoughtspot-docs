...
We currently support connections to Azure OpenAI, Google Vertex AI, and custom LLM gateways. We also support Snowflake Cortex as a native LLM provider, which allows you to leverage link:https://docs.snowflake.com/en/sql-reference/functions/complete-snowflake-cortex[LLM hosted by Snowflake] within your existing Snowflake security perimeter and infrastructure for generative AI operations within ThoughtSpot.

NOTE: Snowflake Cortex as an LLM provider is secured by Key Pair authentication (JWT-based).
...
* *Azure OpenAI* configurations are automatically migrated to *ThoughtSpot Enabled: Auto Select*. If you wish to use only Thoughtspot managed Azure OpenAI Connections, please configure the LLM configuration to ThoughtSpot Enabled Azure OpenAI.
...
Your endpoint must be fully compatible with the API specification for our connector (for example, the OpenAI-compatible API spec for the LLM Gateway option). Custom or non-compiant APIs will fail.
////
