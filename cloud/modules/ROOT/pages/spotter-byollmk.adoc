...
:linkattrs:
:experimental:
:jira: SCAL-276577, SCAL-321304, SCAL-308243, SCAL-337853
...
Before contacting {support-url}, gather the required credentials for your specific LLM provider. This will expedite the setup and compatibility check.

Your LLM endpoint must be reachable over the public internet. ThoughtSpot does not support private connectivity to LLM providers, such as AWS PrivateLink or Azure Private Link. To restrict access, allowlist the ThoughtSpot LLM Gateway IP addresses in your provider's network policy. Contact {support-url} for the IP addresses.

We currently support connections to Amazon Bedrock, Azure OpenAI, Databricks, Google Vertex AI, Snowflake Cortex, and custom LLM gateways.
...
For steps on creating a long-term API key, see link:https://docs.aws.amazon.com/bedrock/latest/userguide/api-keys-generate.html[Generate an Amazon Bedrock API key^] in the AWS documentation.
...
For steps on how to create an Azure OpenAI resource, see link:https://learn.microsoft.com/en-us/azure/ai-foundry/openai/how-to/create-resource?pivots=web-portal[Create and deploy an Azure OpenAI in Microsoft Foundry Models resource^] in Microsoft's Azure documentation.
...
- Your workspace URL.
- Your API token.
- The model names for your models.
...
For steps on how to create Service account keys, see link:https://docs.cloud.google.com/iam/docs/keys-create-delete[Create and delete service account keys^] in Google Cloud's IAM documentation.
...
- A programmatic access token (PAT) for a user with access to Cortex.
- The model names for your models.
...
.Important notice about Spotter 3
[CAUTION]
====
* Though the LLM configuration section in the *Admin > ThoughtSpot AI* page indicates manual model selection is not supported, BYOLLM configurations for Spotter 3 are supported but considered experimental. As a result, performance and stability may vary.
* Spotter 3 requires you to provide both Claude and GPT LLMs in your BYOLLM configuration and has a dependency on GPT 5.4 using Microsoft Azure for code execution. Contact {support-url} for more information.
* Manual presets for Microsoft Azure and Google Gemini are not supported in Spotter 3 at the moment.
====
...
== Models powering ThoughtSpot AI/ Recommended LLMs

ThoughtSpot AI leverages multiple sets of LLMs to provide users with the best experience. With ThoughtSpot Enabled (Auto Mode), all of these LLMs are provisioned and managed for you. With BYOLLM, to ensure the best experience, we recommend using the following LLMs to be provisioned:

[options="header"]
|===
| Capability | Required model(s)

| Spotter 1 | Claude Opus 4.6
| Spotter 2 | Claude Opus 4.6
| Spotter 3 | Claude Opus 4.5 + GPT-5.4 with code execution
| Other ThoughtSpot AI features (platform-wide) | Claude Sonnet 4.6 and Claude Haiku 4.5 (or equivalent)
|===
