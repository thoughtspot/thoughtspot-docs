...
* You must use a single data source, like a table, Model, or view, to run a proximity search.
You cannot search on two different tables, related by a join, like you can in a typical ThoughtSpot search.
To search on two (or more) different tables, xref:models.adoc[create a Model] that contains data from those tables and use the Model as your data source.
