...
:jira: SCAL-175184, SCAL-202985, SCAL-227091, SCAL-233595 (Mumbai), SCAL-259737, SCAL-264258, SCAL-278731, SCAL-293347
...
ThoughtSpot Cloud on GCP is available in the following regions:
...
* ThoughtSpot Team (multi-tenant), Essentials (multi-tenant) and ThoughtSpot Pro (multi-tenant) are available exclusively on AWS in a region designated by ThoughtSpot.
...
* Customer-specific whitelisting is not supported for Pro multi-tenant and Essentials customers.
* Data cannot be migrated to another ThoughtSpot cluster, but you can migrate Liveboards, Answers, views, tables, and Models.
* The data in the Spotter Conversations Liveboard is not currently available for ThoughtSpot instances hosted in GCP and AWS v2 environments.
+
...
CAUTION: Custom IPs (CIDR blocks) are limited to 200 in allowed origins.
