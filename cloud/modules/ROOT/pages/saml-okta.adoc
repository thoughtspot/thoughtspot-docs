...
Org isolation is non-destructive. If a user already has manually-assigned membership in another Org — granted by a cluster administrator — that membership is preserved after a per-org IdP login. A per-org SAML IdP cannot grant new access to Orgs outside its authorized Org, but it also cannot remove existing, manually-assigned access.
