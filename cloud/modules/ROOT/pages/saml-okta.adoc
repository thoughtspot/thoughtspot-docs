...
You can configure SAML SSO login for more than one Identity Provider. To configure this, contact {support-url}.


////
When the users have more than one IdP, users enter only their username on the login screen. If then they choose to login through SSO, and are not a local user, they are redirected to their default IdP for authentication.
There is no option on the login screen for users to select from the multiple IdPs. To use a non-default IdP, users must initiate login from that IdP’s own login page. These limitations mean the user experience for SAML SSO login with multiple IdPs is not fully flexible and users cannot choose their IdP at login.
////

// SOURCE: SCAL-291968 — OIDCClient.java, OrgUtils.java, SecurityEventTypeEnum.java
[#per-org-saml-isolation]
=== Org isolation for per-org SAML authentication

If your ThoughtSpot cluster uses per-org IdP configuration where each Org is bound to its own identity provider, ThoughtSpot enforces strict org isolation on all incoming SAML group and Org claims.

==== How org isolation works

When a user authenticates through a per-org IdP, ThoughtSpot identifies the *authorized Org* associated with that IdP. It then evaluates every group and Org claim in the SAML assertion:

* Claims that reference the authorized Org are accepted and processed normally.
* Claims that reference any other Org are dropped. Dropped claims do not grant access and are not used for group provisioning in those Orgs.
...
NOTE: Org isolation applies only to per-org IdP logins. Cluster-wide (single IdP) SAML configurations are not affected by this behavior.

==== Effect on existing Org memberships

Logging in through a per-org IdP affects existing Org memberships. When a user authenticates via a per-org IdP, ThoughtSpot evaluates and reconciles the user's Org memberships based on the claims scoped to that IdP's authorized Org.
...
.. For the *Single sign on URL* and the *Audience URI* in your IdP provider, you must enter dummy values. You return to these fields in step 16, after completing SAML configuration in ThoughtSpot.
...
. Go to the *Admin settings* tab > *User management* > *Authentication*.
. Click the *Configure IdP* button.
+
[.bordered]
image::saml-config-idp.png[ThoughtSpot Authentication page]
. Select the IdP, click the *SAML* tile.
+
[.bordered]
image::saml-1.png[Select your IdP]

. Add the following details to configure your IdP:
+
[.bordered]
image::saml-idp-details.png[Configure SAML]
...
+
[.bordered]
image::saml-idp-adv-details.png[Advanced SAML configuration]
Request binding;; Binding used for mapping the SAML protocol message. The default is `HTTP-POST`.
Request signature algorithm;; Signature algorithm used to sign the authentication request to your IdP. The default is `SHA-256`.
...
+
[.bordered]
image::saml-map-attributes.png[Map attributes]
+
[IMPORTANT]
====
Map the mandatory attributes carefully.
The value you enter in each *SAML attribute* box must exactly match the name of the corresponding attribute in your IdP's SAML assertion, including its spelling and case.

If a mandatory attribute is missing, misspelled, or mapped to the wrong assertion attribute, ThoughtSpot cannot resolve the incoming assertion to a user, and sign-in fails for every user who authenticates through this connection.
====
...
. In the *SAML attribute* text box for *Email*, enter the name of the mail/email attribute in your IdP's SAML assertion. This attribute maps to the *Email* field for a ThoughtSpot user, which does not need to be unique.
...
. In the *SAML attribute* text box for *Roles*, enter the role associated to the user.
...
To determine if ThoughtSpot supports your preferred IdP, talk to your ThoughtSpot contact.
. Select *Next*.
. Under *Detail URLs*, collect the information required to add the ThoughtSpot application to your IdP.
...
+
[.bordered]
image::saml-final-step.png[Copy ThoughtSpot information]
...
.. Replace the dummy value you used for the *Single sign on URL* with the *Assertion consumer service URL* provided by ThoughtSpot on the *Detail URLs* page in the SAML configuration.
.. Replace the dummy value you used for the *Audience URI* with the *Audience* provided by ThoughtSpot on the *Detail URLs* page in the SAML configuration.
...
* Navigate to your SAML connection and click the *More* menu image:icon-more-10px.png[more options menu icon] > *Edit*
...
This file contains the public key you need if you want to encrypt your SAML assertions. It also contains the *Assertion Consumer Service URL* and *Audience*. If you did not download the `spring_saml_metadata.xml` file, navigate to the SAML configuration page in ThoughtSpot: *Admin > SAML*. Select *Download metadata*. If you do not see this option, you have not completed configuration of SAML in ThoughtSpot.

IMPORTANT: When configuring SAML 2.0, make sure the SAML user attributes exactly match appropriate fields.
This allows the ThoughtSpot system to automatically pick up certain attributes and subjects, such as a user's email address, display name, and username.
You must ensure that the values you use for these attributes are the same in your IdP and in the *SAML attribute* field in the ThoughtSpot SAML configuration flow.
...
>
> * We highly recommend that you register for the free Authentication course in ThoughtSpot University that covers the information in this article, and watch the https://training.thoughtspot.com/admin-authentication/2499271[SAML Authentication^] lesson.
