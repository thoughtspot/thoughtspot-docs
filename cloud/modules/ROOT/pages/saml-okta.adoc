...
For example, if a user logs in through IdP-A, which is bound to Org X, a claim of `PowerUsers@X` is accepted. A claim of `Admins@Y` is dropped.
...
Org isolation is non-destructive. If a user already has manually-assigned membership in another Org — granted by a cluster administrator — that membership is preserved after authenticating via a cluster-wide (primary org) IdP. A per-org SAML IdP cannot grant new access to Orgs outside its authorized Org, but it also cannot remove existing, manually-assigned access.
