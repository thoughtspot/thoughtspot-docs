...
:jira: SCAL-222443, SCAL-266780, SCAL-240707, SCAL-240706, SCAL-292858
...
+
Because this is an approximate date filter, it is designed to apply a predicate that applies a filter that returns slightly more data than the dynamic period.
+
That means:
+
--
...
+
This option is applicable when the data model includes a dimensional date table. That is, this CTE pattern will not be invoked if the date column is from the fact table itself. To enable this option, contact {support-url}.
...
Respect join definitions for RLS rules:: The default setting is FALSE. When FALSE RLS rules are applied as an INNER join. When TRUE the join condition defined on the logical tables will be respected by the RLS rules. For more information, see xref:security-rls.adoc#respect[Respect join definitions].
...
To implement column-level security, this setting should be set to `TRUE`. This results in a stricter object security Model. In short, all data objects must be shared with consumers rather than just Models. For assistance, contact {support-url}.
For additional information, refer to link:https://docs.thoughtspot.com/cloud/latest/security-data-object#_object_security[this article^].
