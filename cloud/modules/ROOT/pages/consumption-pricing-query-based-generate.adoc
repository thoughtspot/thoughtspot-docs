...
NOTE: User actions that are performed against an Analyst Studio dataset are accounted for using the same manner as other data sources.
...
- Inspecting data in Data samples tabs after adding multiple tables in a Model with joins between them
...
. Select the *Edit chart configuration* icon image:icon-gear-10px.png[chart configuration icon].
