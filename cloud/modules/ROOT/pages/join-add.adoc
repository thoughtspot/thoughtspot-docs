...
:page-last_updated: Sept 24, 2026
:linkattrs:
:experimental:
...
[#change-join-cardinality]
==== Change join cardinality

You can now change the cardinality of a join (1:1, 1:Many, Many:1) without needing to delete and remake the join.

To change the cardinality of a join, open the source in the *Data* tab and navigate to the joins tab. Click the *Edit* icon image:icon-edit-10px.png[pencil icon] in the top right corner, scroll to the *Cardinality* section of the *Edit join* pop-up, and click the desired cardinality. Click *Save*, review the changes that will be made to the join's dependents, and click *Save* again.
...
NOTE: If you create joins at the table level, and then create a Model that uses the columns from the table, the settings are inherited from the table at the point in time that the Model is created.
...
////
==== Creating a join from a Model

NOTE: You can only create a join between a Model and an uploaded CSV table.

include::partial$joins-create-Model.adoc[]
////
...
////
==== Editing a join from a Model

include::partial$joins-edit-Model.adoc[]
////
...
IMPORTANT: Deleting joins that have dependencies is not supported.
