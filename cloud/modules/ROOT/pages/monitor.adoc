...
:experimental:
:jira: SCAL-127727, SCAL-89341, SCAL-173345, SCAL-164086, SCAL-201032, SCAL-201033, SCAL-207062, SCAL-208519, SCAL-243597
...
NOTE: Anomaly alerts are in Early Access and off by default. Since this feature is early access, we do not support webhook and TML functionality. If you create an anomaly alert, and anomaly alerts are subsequently disabled from your cluster, the anomaly alert will be disabled. If you create an anomaly alert, and your cluster is downgraded from 10.1.0.cl, the anomaly alert becomes a scheduled alert.
...
. For threshold anomaly alerts, select *Values of an attribute crosses a set limit*. Click *Next*.
...
+
NOTE: If new values are added to the attribute you select after the alert is created, ThoughtSpot automatically adds them to the alert.
+
[.bordered]
image::kpi-alert-attribute-select.png[]
...
. For scheduled anomaly alerts, select *Regular updates on values of an attribute*. Click *Next*.
