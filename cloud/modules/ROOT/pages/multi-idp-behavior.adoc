...
[NOTE]
====
If your cluster has Org-scoped IdP connections enabled (available from 26.7.0.cl, currently in Beta), the automatic IdP discovery redirect described in this section is disabled. Login page rendering is controlled per Org. See xref:oidc-org-scoped-idp.adoc[Org-scoped IdP connections and login page rendering].
====
...
+
NOTE: The org name is case-sensitive. The per-org subdomain URL is derived from the org name exactly as it is stored. The org must be named `primary` (all lowercase) for the subdomain `primary.<cluster-name>.thoughtspot.cloud` to resolve correctly.
+
...
For users on ThoughtSpot instances 26.7.0.cl or later, see xref:oidc-org-scoped-idp.adoc[Org-scoped IdP connections].

[#troubleshoot-per-org-redirect]
...
> **Related information**
>
> * xref:oidc-org-scoped-idp.adoc[Org-scoped IdP connections]