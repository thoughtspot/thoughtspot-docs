...
When you search, ThoughtSpot can automatically suggest a chart type based on your search query. This is how it works:

* *New queries*: When you submit a query for the first time, ThoughtSpot analyzes the data and automatically selects the most appropriate chart.
* *Follow-up queries*: If you modify an existing query by adding or removing fields, ThoughtSpot should retain the existing chart type _unless_ constraints force it to change.
* *Drill-downs*: When you drill down into data, ThoughtSpot ensures a predictable user experience by maintaining the current chart type wherever possible.

== Agentic charting in Spotter

For charts generated automatically in Spotter conversations from code execution, web data, and SQL queries, see xref:charts-for-agentic-answers.adoc[Charts for agentic answers].
...
ThoughtSpot renders a horizontal scrollbar across all chart types. For pivot tables, this removes the previous row-header column width cap, so attribute labels render at their full natural width instead of being truncated. The scrollbar behavior is consistent across chart types and viewport sizes.

NOTE: This feature requires New Charts to be enabled.
...
[#parameters]
== Dynamic parameters in charts and Answers

You can add a parameter name directly in chart components like axis labels and column names, as well as the title and description of an Answer. When you select a different parameter value in the Answer, the data and contextual information (axis names, column names, and legends) change to reflect the parameter selected. Any place a parameter is used in Liveboards or Answers automatically displays the current information associated with the selected parameter value.

+++
<div class="border">
<video autoplay loop muted controls width="100%" controlsList="nodownload">
<source src="https://docs.thoughtspot.com/cloud/latest/_images/dyn-params.mp4" type="video/mp4">
</video>
</div>
+++

=== Adding parameters to an axis name in a chart

To add a parameter to an axis name of a chart, do the following:

. While viewing your Answer in chart view, select the axis icon image:chart-axis.png[chart configuration icon,18] in the right pane.
. Select the X or Y-axis.
. In the Axis name field, enter an ampersand (@) to display the list of parameters available to you in your Answer.
+
[.bordered]
image::chart-params-axis.png[Chart parameters]

. Click the parameter you want to use.
. Add any additional parameters to other axis names, if needed.

=== Adding parameters to a column name in a chart

To add a parameter to a column name of a chart, do the following:

. While viewing your Answer in either chart or table view, select the column configuration icon image:chart-col.png[chart configuration icon,18] in the right pane.
. In a column name field enter an ampersand (@) to display the list of parameters available to you in your Answer.
+
[.bordered]
image::chart-params-col.png[Chart parameters]

. Click the parameter you want to use.
. Add any additional parameters to other column names, if needed.

=== Adding parameters to the title or description of an Answer

To add a parameter to the title or description of an Answer, do the following:

. While viewing your Answer in either chart or table view, click the title or description.
...
>
> * We highly recommend that you register for the free Customizing Charts and Visualizations course in ThoughtSpot University that covers the information in this article, and watch the following lessons:
> ** https://training.thoughtspot.com/customizing-charts-and-visualizations/1998314[Advanced Chart Settings^]
> ** https://training.thoughtspot.com/customizing-charts-and-visualizations/2439046[Using Pivot Tables^]
