...
You can now migrate feedback between Models or Worksheets using the feedback TML file. This allows you to easily transfer coaching between Models or Worksheets without needing to train Spotter for each Model or Worksheet individually.  You must have edit access to a Model or Worksheet to export or import feedback. To enable this feature, contact {support-url}.
...
* Check for conflicts within the file– no two feedback objects can have the same combination of `feedback_phrase` and `parent_question`. If the target Worksheet or Model has existing feedback entries matching the feedback TML for the above combination, remove the common feedback entries to proceed with migration.
