...
It is important to define the outer aggregation when calculating ratios. The legacy behavior would include a default aggregation. As the new behavior respects the fixed grouping column, this should be defined in the formula.
...
As the legacy behavior did not respect the fixed column attributes, formulas were written to simulate this behavior. The new query pattern requires that the data analyst explicitly defines this optional grouping.
