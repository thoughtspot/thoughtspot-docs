...
* Ask your ThoughtSpot administrator to disable *Index columns*, then manually set the Index Type on columns that would be helpful when searching.
+
To disable *Index columns*, ask your ThoughtSpot administrator to do the following:

. Navigate to *Admin* > *All Orgs*.
. Under _Application settings_, select *Search & SpotIQ*.
. Next to _Search settings_ click *Edit*.
. Disable *Index columns (effective for new tables/columns only)* by clicking the status slider to set the feature to image:icon-slider-toggle-disable-20px.png[Early access features toggle] *Disabled*.
. Click Save.
