...
moving_average ( [sales_amount] , 4 , 0 , [order_date] ) calculates the 5 month moving average of sales (note the window is of 4 previous values plus the current value).
...
Some of the computations listed above will not be supported with aggregated fields. For example, if the analysts creates a group aggregate formula at worksheet level then tries to use the same in creating another group aggregate formula at answer level then those things are not supported.
