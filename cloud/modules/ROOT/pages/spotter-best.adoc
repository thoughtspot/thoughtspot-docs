= Questions that Spotter can answer
...
== How Spotter answers your questions

Spotter translates natural language instructions to analytical queries on your data model that can help you perform some complex analysis. Let’s take a look at the capabilities of Spotter in more detail by taking a simplified table given below.
...
Mathematical computations:: Spotter can count things in your data set (for example, `how many regions are there?` ) , it can aggregate metrics (for example,  `what is the average sales?` ),  or it can perform complex mathematical computations (for example, `how do the sales for the east region compare against sales of the west region?`).


Grouping:: Spotter can help you perform the computations for specific groups (for example, `what are the total sales by region?`, or `what are the maximum sales for each region?`). You can also ask Spotter to create grouping on date columns (for example, `what are the yearly sales?` or `what are the monthly sales?`).
...
Basic aggregations allow you to compute summary statistics such as sum, average, count, or unique count on measure columns, optionally grouped by attributes or dates. Use these aggregations to gain a clear understanding of your data at a summarized level.
...
| Calculates the total number of purchase transactions across your entire data set, providing immediate insight into overall sales volume.
...
Spotter offers powerful and reliable date-based analysis capabilities.
...
| Show me total sales from 2020 start until 2023 end.
...
These functions enable grouping and aggregation of data based on different levels of date granularity, such as by month, week, day of the week, or other date buckets.
...
| What are the average sales for each month of this year?
...
Subqueries allow you to embed a query within another query, using the results of the inner query to filter or condition the outer query. This capability enables complex data retrieval and analysis. By leveraging subqueries, you can perform advanced tasks such as identifying items based on intricate criteria, comparing specific subsets of data, and filtering results based on dynamically computed values. Subqueries are particularly powerful when used with operators like `in` and `not in` to filter based on lists of values generated by the inner query.
...
| What are the total sales of jackets only from the top 5 cities by sales?
...
| Identifies potentially under-performing or obsolete inventory by highlighting products with no recent sales activity, enabling inventory optimization decisions.
...
Growth calculations enable you to determine percentage changes of measures over time. This feature allows you to analyze trends, identify periods of significant increase or decrease in key metrics, and understand the rate at which your business is evolving. Spotter supports various time granularities, including yearly, quarterly, monthly, weekly, and daily, as well as period-over-period comparisons like year-over-year or month-over-month.
...
| `[total sales with 20% increase] [region = 'east']`
...
| `[sales] [performance category] [store] [date = last month]`
