...
This article is an overview of the Orgs functionality and Org management in the standard ThoughtSpot Cloud environment. To learn more about Orgs functionality and programmatic Org management using REST APIs in the ThoughtSpot developer environment, refer to https://developers.thoughtspot.com/docs/?pageid=orgs[Multi-tenancy with Orgs^]. To learn more about Orgs APIs, refer to https://developers.thoughtspot.com/docs/?pageid=orgs-api[Orgs API^].

NOTE: After you enable the Orgs feature on a cluster, you can’t turn off the ability to create Orgs. However, your environment remains a single-tenant environment until you create an Org. You can also delete all the Orgs you created, and just use the Primary Org, if you would like a single-tenant environment with Orgs enabled. For more information, see <<enable-disable,Enabling and disabling Orgs>>.
...
* ThoughtSpot objects: Liveboards, Models, Answers, and tables. You cannot share objects between Orgs.
* Search data content and indexing
* Search Answers content and indexing
* Other indexing

[#sso-org-isolation]
=== SSO claim isolation for per-org IdPs

When your cluster uses per-org IdP configuration with IAMv2, ThoughtSpot extends multi-tenant isolation to SSO authentication. Incoming SAML and OIDC group and Org claims are evaluated at login time:

* Claims that reference the IdP's authorized Org are accepted.
* Claims that reference other Orgs are dropped and recorded as security audit events.
* Unsuffixed group claims are automatically scoped to the authorized Org.

This prevents an IdP administrator in one Org from using group assertions to gain access to another Org. Existing manually-assigned Org memberships are not affected.

For configuration details, see xref:saml-okta.adoc#per-org-saml-isolation[Org isolation for per-org SAML authentication] and xref:oidc-iamv2.adoc#per-org-oidc-isolation[Org isolation for per-org OIDC authentication].
...
[#admin-visibility-orgs]
=== #Cluster admin account visibility across Orgs#

When a cluster administrator switches between Orgs, ThoughtSpot displays only the admin account used to log in to the current session. Other cluster admin accounts are not shown in non-Primary Orgs.

This behavior applies to both local and SSO/SAML cluster admin accounts:

* If you log in with a *local* cluster admin account, only that local account is visible when you switch between Orgs.
* If you log in with an *SSO/SAML* cluster admin account, only that SSO account is visible when you switch between Orgs.

[NOTE]
====
If you notice that a specific cluster admin account is not visible in a non-Primary Org, verify which account was used to start the session. Only the session account is displayed across Orgs, this is by design. To see a different cluster admin account, log out and log in with that account.
====
...
* Monitor alerts
* Liveboard schedule
* Liveboard access emails
* Digest emails

NOTE: You can now open and view links from different Orgs in different tabs or windows without logging in or out of the Org that you started in.
If you open a link from an Org other than the one that you are currently logged into, and then log out, you will be directed to the Org that you were previously logged into when you return.

NOTE: The *Admin* page can only be accessed when you are logged in to the appropriate Org.
...
. Select *All Orgs*.
. Under *Application settings*, select *Administration*.
...
The Orgs feature requires either the ThoughtSpot Enterprise Edition or the ThoughtSpot Embedded Enterprise Edition. See https://www.thoughtspot.com/pricing[ThoughtSpot pricing and editions^]. Orgs are enabled by default on all new clusters. To enable Orgs in your environment, contact {support-url}.
...
>
> * We highly recommend that you register for the free Multi-Tenancy with Orgs course in ThoughtSpot University that covers the information in this article, and watch the following lessons:
> ** https://training.thoughtspot.com/admin-logical-multi-tenancy-with-orgs/2499344[Introduction to Multi-Tenancy with Orgs^]
> ** https://training.thoughtspot.com/admin-logical-multi-tenancy-with-orgs/2499359[RBAC with Orgs^]
