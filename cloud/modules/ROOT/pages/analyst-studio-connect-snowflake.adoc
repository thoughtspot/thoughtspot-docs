...
NOTE: Snowflake currently blocks single-factor password sign-in, except for service users. We recommend using Key Pair authentication.
...
Please follow the link:https://docs.snowflake.com/en/user-guide/key-pair-auth#configuring-key-pair-authentication[Snowflake documentation,window=_blank] for configuring key pair authentication. We recommend key pair authentication, as it has no expiry.
...
CREATE OR REPLACE SECURITY INTEGRATION <enter a name for your security role>
  ...
  OAUTH_REDIRECT_URI = 'https://<public url of your AS instance>/snowflake_oauth_v1/callback'
...
CREATE OR REPLACE SECURITY INTEGRATION OAUTH_CONFIG
  ...
  OAUTH_REDIRECT_URI = 'https://app.as01.analyststudio.thoughtspot.cloud/snowflake_oauth_v1/callback'
  OAUTH_USE_SECONDARY_ROLES = IMPLICIT
  ENABLED = TRUE
...
Users see a prompt to authenticate a connection when using an Internal OAuth connection for the first time, or when the token expires.
You must periodically re-authenticate connections to Snowflake made via OAuth with PKCE.
