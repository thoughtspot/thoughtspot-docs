= TML for Models
:page-last_updated: May 5, 2026
:author: Rani Gangwar, Naomi
:linkattrs:
:experimental:
:page-aliases:
...
:object: Models
:jira: SCAL-158490, SCAL-186188, SCAL-206010, SCAL-210659, SCAL-220585, SCAL-231618, SCAL-220584, SCAL-220587, SCAL-247373, SCAL-250091, SCAL-256366, SCAL-264258, SCAL-266780, SCAL-280284, SCAL-287776, SCAL-279451, SCAL-287350

include::partial$tml-intro-models.adoc[]

include::partial$tml-syntax-mandatory.adoc[]

[#date-bucket-default]
include::partial$tml-date-bucket.adoc[]
...
To use the data panel column groups properties, you first add a property to each column properties definition, allocating each column to a group. This property, <<data_panel_column_groups,data_panel_column_groups>>, defines which group(s) a column belongs to. To assign a column to multiple groups, use a comma-separated list.

Use the following syntax:

----
- name: Transactiondate
    column_id: DIMDATE::TRANSACTIONDATE
    properties:
      column_type: ATTRIBUTE
      index_type: DONT_INDEX
    data_panel_column_groups: {Dates:}
----
...
column_groups:
  - type: DATA_PANEL
    ...
    - name: Stores
      include_ungrouped_columns: false
    - name: ungrouped columns
      include_ungrouped_columns: true
    - name: Customers
      include_ungrouped_columns: false
----

In the above syntax, the <<status,status>>: Enable property enables the column groups feature. You can change the status to Disable to remove the column groups from the data panel without deleting the definition in the TML file.

The <<default_sort,default_sort>> property keeps the order set in the TML file when disabled. When enabled, the column groups will be listed in alphabetical order in the Data panel.

Define the column group names and contents using the <<column_group_info,column_group_info>> property. The <<name,name>> property defines the group name. The <<include_ungrouped_columns,include_ungrouped_columns>> property defines which group any columns you did not define with the <<data_panel_column_groups,data_panel_column_groups>> property are allocated to. There must be one column group with the include_ungrouped_columns property set to true.
...
<<is_time_bucket_restricted,is_time_bucket_restricted>>
...
[#apply_on_tables]
apply_on_tables:: Use this property, also known as _Progressive Model filters_, to define on which tables the filter will be applied. When used, any search using a column from that table will include the filter. Searches on other tables within the {model} will not include the filter.
...
[#condition]
condition:: Section header for the rules of the constraint.

[#constraint]
constraint:: Section header for the constraint rule. This may be a repeating element to define each rule.

[#constraints]
constraints:: Section header to define TML constraints. There can only be a single constraints section.
...
date_range_condition:: Defines that the condition is based upon a date range. Note that this is currently the only condition that is supported.
...
[#duration]
duration:: An integer value to define the length of the period. This is used in conjunction with <<bucket,bucket>> to define the length of time.
...
[#is_mandatory]
is_mandatory::
Marks a Model column as mandatory to all searches. When set up, users searching on this Model see the mandatory column included in any search they begin, even if they do not manually add it. Users can manually add a mandatory column to the search bar if they use an extra qualifier, such as aggregation (sum, average, etc.) or a date bucket (monthly, yearly, etc.).
+
NOTE: To enable mandatory columns, contact {support-url}.
...
[#is_spotter_enabled]
is_spotter_enabled:: Defines whether Spotter is enabled on the Model. If Spotter is not enabled in your instance, this property is titled `is_sage_enabled`.

[#is_time_bucket_restricted]
is_time_bucket_restricted:: When defined as "true" for a column, only the defined default time bucket is shown in the bucketing menu.
+
NOTE: To enable this property, contact {support-url}. For time bucket restriction to be enabled, default_date_bucket must be set.
...
Default: `true`
+
See xref:model-progressive-joins.adoc[How the Model join rule works].
