...
:linkattrs:
:page-aliases:
:description: ThoughtSpot Cloud 10.10.0.cl release notes
...
[#alerts-by]
Alerts by attribute [.badge.badge-ga-relnotes]#GA#:: Alerts by attribute are now available to all users and on by default. You can now set KPI alerts to trigger when a selected attribute changes. This can include changes in the value of the attribute, such as an increase in sales by percentage by state, or a scheduled alert to trigger on a weekly basis.
+
...
When Spotter Agent is enabled, you can give feedback with additional details, and leave comments that are reviewed by your administrator. The feedback you give will appear in the Spotter Conversations Liveboard. To enable this feature, contact {support-url}.

// Naomi. docs jiraL SCAL-258753
[#spotter-analytics]
Spotter analytics model changes [.badge.badge-ga-relnotes]#GA#:: We made the following changes to the TS: Spotter analytics data model, which is used by the Spotter Conversations Liveboard:
+
--
...
[#monitor-application]
Monitor application of row-level security at database level [.badge.badge-ga-relnotes]#GA#:: When opening SQL comments in your database, you can now see whether RLS was applied to specific queries. In the SQL comments, you can check whether the value for `isRLSApplied` is true or false.


// Mary. jira: SCAL-245938. docs jira: SCAL-255650
[#spotter-conversations]
Spotter Conversations Liveboard available in all Orgs [.badge.badge-ga-relnotes]#GA#:: The Spotter Conversations Liveboard is now available by default in all Orgs. In the primary Org, only administrators have access by default.
