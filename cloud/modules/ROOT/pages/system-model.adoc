= System Models, tables, and views
:page-last_updated: July 22, 2026
:author: Rani
:linkattrs:
:experimental:
...
:jira: SCAL-201655, SCAL-71323, SCAL-246579, SCAL-264258, SCAL-287601
...
MetricsMonitoring:: This table contains data on how users are searching for existing Answers and Liveboards. It contains information on what users are searching for, where users are successful and where they are not, based on click count and position. This table is a source for the Discover Monitoring Data Model.
TS: Entitlement Tracking:: Sources data from Snowflake via a secure view and refreshes every 24 hours to track entitlements for the products purchased by customers. This system model is automatically provisioned on all ThoughtSpot Cloud instances and shared across all Orgs on the instance by default.
TS: Object tracking [.badge.badge-beta-relnotes]#Beta#:: Contains creation date, creator, last modification date, and last modifier for any supported Liveboard, Model, and Answer. Administrators can leverage this data model to build custom monitoring Liveboards, offering visibility into who created or modified objects or models, and when these actions occurred.
TS: Login Activity Tracking:: This model provides visibility into how users access the system. It contains username and ID, Org name and ID, login timestamp, whether the login was successful or not, https response code, and login success rate.
TS: Provisioned Users:: Contains data related to the current status of the user (active, inactive, or deleted), Org ID and Org name as in ThoughtSpot, user ID, and the number of provisioned users.
TS: AI and BI Stats :: Contains query performance metrics for each query made against an external database. It is based on Query Stats, derived from Data manager. It offers a stable and trusted alternative to other data sources. This data source is already utilized for billing and has demonstrated reliability. This model is enabled by default. For more information, see xref:data-workspace.adoc#aibistats[AI and BI Stats]
...
>
> * We highly recommend that you register for the free Monitoring Performance course in ThoughtSpot University that covers the information in this article, and watch the https://training.thoughtspot.com/admin-monitoring-performance/2499318[System Models^] lesson.
