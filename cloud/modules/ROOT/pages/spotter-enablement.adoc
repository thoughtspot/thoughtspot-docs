...
:jira: SCAL-211072, SCAL-215955, SCAL-256741, SCAL-264258, SCAL-276222
...
. From the *Choose Spotter answer experience* section, you can choose the answer experience as *Spotter agent* or *Classic experience*. You can also set Spotter Agent as on by default, if it has been enabled. Enabling *Enable add to coaching from chat* adds the *+Add to coaching* button so you can add reference questions and business terms directly from your Spotter conversation.
//For more information on the behavior of Spotter or Sage and Ask Sage in relation to Spotter Agent, see xref:spotter-agent.adoc[Spotter Agent].
+
[#why]
...
+
NOTE: You must enable Spotter Agent to enable why questions.
...
Natural Language Instructions:: Adds the information pane icon in Spotter, which allows you to provide Spotter with global rules that it should follow across all relevant queries. A natural language instruction is a standalone directive designed to teach the system a core concept, a default behavior, or a specific data nuance.
