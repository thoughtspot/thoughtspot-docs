...
When using Spotter 3, query results are cached for up to six hours by defualt as part of Spotter’s Advanced Analysis capabilities, which is used for performing user-specific and requested analytics. If saved chats are enabled, query results persist for as long as the set chat history retention period (maximum 180 days).
