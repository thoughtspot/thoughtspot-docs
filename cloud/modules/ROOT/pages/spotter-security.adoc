...
:jira: SCAL-264258, SCAL-287506, SCAL-326962
...
When using Spotter 3, Spotter also uses data from a data warehouse query response, with the information above, to provide more intelligent, detailed, and contextualized analysis grounded in your data.
...
|Data training
2+|_No customer data is used for LLM training._


|Data persistence and retention
|Customer data is not persisted or cached.

When using Spotter 3, query results are cached for up to six hours by default as part of Spotter’s Advanced Analysis capabilities, which is used for performing user-specific and requested analytics. If saved chats are enabled, query results persist for as long as the set chat history retention period (maximum 180 days).
