...
:jira: SCAL-229666, SCAL-238614, SCAL-247618, SCAL-264258, SCAL-270875
...
== Understand and use sets in ThoughtSpot
Sets help you define and reuse groups of data for flexible analysis.

* Column Sets allow you to categorize values in a column, such as grouping products into categories, creating bins for numerical values or defining festive periods for seasonal analysis.

* Query Sets enable advanced filtering and aggregation across multiple columns, helping you conduct ABC Curve Analysis, Basket Analysis, Frequency Analysis, and Segmentation Analysis. You can also apply formulas, rankings, and time-based filters for insights like Acquisition Cohort Analysis and Ratio Contributions in Basket Analysis.

Use Sets to simplify queries, enhance filtering, and power interactive visualizations in your analysis.
...
. Navigate to the *Search Data* page by clicking *Search Data* on the *Insights* tab.
...
+
[.bordered]
image::query-set-column.png[Query set column]
...
****
You can now build a set for a specific Answer. An Answer-level set does not need to be reusable. Sets at the answer level allow ThoughtSpot to answer query-on-query questions by generating and consuming Query sets within a single Answer flow. Users with edit permissions on an Answer can view and modify the associated Sets.

NOTE: These sets are Answer-specific and only saved if the Answer object is saved.

To create a set at the Answer level, follow these steps:

. Navigate to the Answer you want to create a set for.
. Click *Add* and select *Query set*.
+
[.bordered]
image::query-set-add.png[Add query set]
. Select a column from the left side bar, or type a column name into the search bar.
. Under *Sets* select *Answer* to create an Answer-level set.

[.bordered]
image::answer-level-set.png[Answer-level set]
...
>
> * We highly recommend that you register for the free Introduction to Sets course in ThoughtSpot University that covers the information in this article, and watch the https://training.thoughtspot.com/introduction-to-sets/2009557[Creating Query Sets^] lesson.
