...
:jira: SCAL-212095, SCAL-233479, SCAL-236631, SCAL-235082, SCAL-254146, SCAL-255171, SCAL-244854, SCAL-269969, SCAL-281477, SCAL-245724, SCAL-220584, SCAL-303376
...
The Bridge Client must make *outbound TCP connections* to Bridge Server on *port 443* or *port 8444*.
...
*What are the hardware requirements for Bridge connections?*:: We recommend the machine running Bridge client have at least 1 Gbps bi-directional network bandwidth. Two CPUs with 1 GB or RAM should suffice. There are no disk space requirements other than enough space to install the client.
...
*How are the credentials protected on the Bridge Client?*:: The Bridge Client doesn't contain any database or CDW credentials. All Database-related credentials or configurations are stored in ThoughtSpot’s application layer (Connections). This is independent of using Bridge or any other connectivity mechanism, such as PrivateLink or VPN.
+
...
Where are the Bridge logs stored?:: The log location varies based on the deployment method you used.
For the docker container, they are accessible within the container logs.
For Linux Package Installation, the default path keeps them in `/opt/mode/bridge/bridge.log`.
If you used RHEL/CentOS, logs are managed and viewable using journalctl.
