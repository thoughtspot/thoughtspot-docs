...
The email also goes to your administrators, anyone with *edit* access for the Liveboard or Answer.

A message appears, telling the user who owns the Liveboard or Answer.
ThoughtSpot sends an email notification, requesting view or edit access, to the Liveboard or Answer owner and up to 10 users with edit access for the Liveboard or Answer. (Cluster administrators are notified only if they also have edit access to the object. Notifications go only to users who have their 'notify on share' preference turned on and a valid email address.)
...
ThoughtSpot sends an email notification, requesting view or edit access, to the Liveboard or Answer owner, anyone with *edit* access on the Liveboard or Answer, all cluster administrators.
The email includes the user's optional message.
...
To request access to a data source, do the following:

. Select the locked icon image:icon-locked-10px.png[Locked icon image] next to the Liveboard or Answer name.
. (Optional) Add a message describing why you want access.
+
You can see which data source you need access to by selecting the down arrow icon next to *Data sources*.
+
...
+
After you select *Request access*, the system tells you who it emailed to request access.
ThoughtSpot sends an email notification, requesting data access, to the Liveboard or Answer owner and up to 10 users with edit access for the Liveboard or Answer. (Cluster administrators are notified only if they also have edit access to the object. Notifications go only to users who have their 'notify on share' preference turned on and a valid email address.)
The email includes the user's optional message.
Any user who received the request can then provide the user with data access.
+
NOTE: Previously, when a user shared a Liveboard with another user who did not request access to the Liveboard's underlying data source, an email was sent to administrator(s). That email is no longer sent.
