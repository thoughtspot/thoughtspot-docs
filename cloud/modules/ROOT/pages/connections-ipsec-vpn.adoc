...
* You must complete a network architecture review.
...
* *Pre-shared secret key (PSK):* A string that is used to encrypt traffic.
...
* *[Optional] NAT (Network Address Translation) IP:* All TCP connections coming from ThoughtSpot into your network have this address as source IP address. This must not collide with any existing network resourc.
By default, if no value is specified, ThoughtSpot will allocate a Public IP address to avoid routing conflicts.
* *[Optional] BGP ASNs (Border Gateway Protocol Autonomous System Numbers) - remote and local:* If you require dynamic routing, you must provide the ASN value for your BGP server. ThoughtSpot will only advertise a single IP address: the NAT IP.
...
VPN gateway configuration varies greatly depending on your network architecture. Configure the VPN gateway using the *Public IP* provided by ThoughtSpot.
...
You can ask {support-url} for the following advanced configuration options:

* VPN instance type. For example, {support-url} can set the instance type to `t2.small`.
* Custom configuration for https://docs.strongswan.org/[strongSwan^].
* Custom configuration for https://bird.network.cz/?get_doc&f=bird.html&v=20[BIRD2^].
