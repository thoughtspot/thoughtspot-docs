...
:page-last_updated: Feb 14, 2025
:author: Naomi, Mark
:linkattrs:
:experimental:
:page-aliases: spotter-worksheet-model.adoc
:description:
:jira: SCAL-228500, SCAL-232768, SCAL-243503, SCAL-240422, SCAL-251986, SCAL-264258, SCAL-269995, SCAL-300551




Set yourself up for success by optimizing your data model configuration. By taking the time to prepare your data for use with our AI systems, you gain more accurate responses to your questions.




There are several steps you can take to optimize Models and significantly improve Spotter accuracy. These steps include:
...
Best practices for Spotter Models include paying attention to column names (making sure column names are intuitive and accurate to the terminology your business users use), synonyms (adding relevant synonyms for column names where users might use different terms for the same thing), column availability (ensuring that all data columns required to answer user queries are present in the Model, and that you don’t have excess columns that aren’t relevant), and column descriptions (providing clear column descriptions to aid Spotter’s understanding).


== Prefer wider tables versus longer tables

This approach helps structure your data to provide the semantic model with more meaningful context.

Long table structure (less recommended):: In a long table, you have generic "metric" and "value" columns. The actual business metric (for example, sales, profit) is stored as a value within a row.
Example;;
+
[options="header"]
|===
...
| 2023-01-01 | Profit | 20

| 2023-01-01 | Quantity | 50
|===

Wide table structure (recommended):: The recommended approach is to flatten the table so that each distinct business metric becomes its own column.
Example;;
+
[options="header"]
|===
...
. Generate xref:spotter-ai-context.adoc[AI Context]. This feature uses your descriptions, column names, and values.
...
. In the Model, click the *More* menu image:icon-more-10px.png[more menu icon image], and select *Enable Spotter*.
...
////
=== Use Spotter optimization to learn from Liveboards [.badge.badge-beta]#Beta#

Learning from Liveboards helps Spotter learn reusable context from selected Liveboards and apply that context while answering questions on related data models.
...
For more information, see xref:spotter-liveboard-learn.adoc[].
////
...
- The "Optimize for Spotter" guidance message appears when there are additional steps recommended for Spotter optimization.
+
[.bordered]
...
>
> * We highly recommend that you register for the free Spotter Administration course in ThoughtSpot University that covers the information in this article, and watch the https://training.thoughtspot.com/spotter-administration/2499379[Setting Up Models for Spotter^] lesson.
