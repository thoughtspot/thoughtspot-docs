...
:page-last_updated: Jul 9, 2025
:author: Naomi
:linkattrs:
:experimental:
:page-layout: default-cloud-no-index-early-access-spotter-3
:description:
:jira: SCAL-259931, SCAL-279825, SCAL-275913, SCAL-280195
...
IMPORTANT: Enabling this feature allows Spotter to send query-related data, such as the involved metrics and dimension values (like "sales" or "region = West"), to an LLM to identify the data points and generate explanations. Please ensure this complies with your organization’s policies. The data is not utilized for model training, and we maintain a zero-data retention policy with all our LLM providers.




NOTE: To enable this feature, contact your administrator. For more information on how to enable why questions in Spotter, see xref:spotter-enable.adoc#why[AI feature enablement].
...
You can build upon an existing surface-level query or existing visualization by choosing a data point of interest and asking a natural question starting with “Why”. Spotter uses change analysis to answer why questions about a single metric on simple, non-sliced charts. Below are a few examples:
...
+
[.bordered]
image::spotter-why-month.png[Ask why there was a drop in sales last month.]
...
* *Detailed visualizations*: Spotter then provides detailed change analysis charts that break down the contributions from different attributes.
+
[.bordered]
image::spotter-answer-why.png[Spotter shows a chart breaking down sales by city.]
...
+
[.bordered]
image::spotter-narrative-summary.png[Spotter summarizes the reasons for the change.]
...
Change analysis:: Spotter does not support change analysis for queries based on the  `growth of` and `versus` (`vs`) keywords. Change analysis is not supported on certain complex formulas based upon group_* functions (such as `sum(group)` or `sum(x)/ group_*`). Simple functions that only use `group_*` are supported.

Personalized columns:: If you have set up xref:spotiq-change.adoc#remember-personalized[personalized columns] in a KPI chart, then ask a why question of the chart in Spotter, Spotter chooses columns based on usage-based ranking to answer, rather than your selected columns.
