...
[NOTE]
To configure OIDC authentication with ThoughtSpot, contact {support-url}.
...
[#per-org-oidc-isolation]
=== Org isolation for per-org OIDC authentication

If your ThoughtSpot cluster uses per-org IdP configuration — where each Org is bound to its own identity provider — ThoughtSpot enforces strict org isolation on all incoming OIDC group and Org claims.

When a user authenticates through a per-org IdP, ThoughtSpot identifies the *authorized Org* for that IdP and evaluates every group and Org claim in the token:

* Claims that reference the authorized Org are accepted and processed normally.
* Claims that reference any other Org are dropped and logged as a security audit event
// (`OIDC_CROSS_ORG_CLAIM_DROPPED`).
* Group claims that carry no Org suffix are automatically scoped to the authorized Org.

Authenticating through a cluster-wide (primary Org) IdP does not alter existing group memberships after a per-org OIDC login.

NOTE: Org isolation applies only to per-org IdP logins. Cluster-wide (single IdP) OIDC configurations are not affected.
...
OIDC is an identity layer on top of OAuth 2.0 that allows secure exchange of user authentication and authorization data between trusted partners.
...
.. For the *Callback URI* in your IdP, you must enter dummy values. You return to these fields after completing OIDC configuration in ThoughtSpot.
...
. Go to *Authentication* and select *Identity Providers* from the side navigation bar.
. Click the *+ Add Identity Provider* button.
. Select the *OIDC IdP* tile.
+
...
Issuer:: URL that uniquely identifies the authorization server. It issues identity tokens and validates user authentication.
