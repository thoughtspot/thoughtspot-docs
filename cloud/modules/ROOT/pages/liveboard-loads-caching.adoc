...
:description: Liveboards use browser caching to load faster. SCAL-294066, SCAL-309554

[.lead]
Learn how to enable browser caching so Liveboards load more quickly.
...
Browser caching must be enabled on your instance by {support-url} before the *Admin Settings* toggle is visible.
...
IMPORTANT: When caching is enabled for a Liveboard, ThoughtSpot is authorised to extract data from the CDW and store it in the user's browser cache on their machine. This is only accessible to the user opening the Liveboard in their browser.


=== Enable browser caching on your instance
To enable browser caching, your ThoughtSpot administrator must do the following:
...
Can I disable caching for a instance?::
Yes. Instance administrators can disable caching for a instance at any time in *Admin Setting*.
