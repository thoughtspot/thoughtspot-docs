...
Before connecting to {connection}, you must have the following permissions:
...
AWS Glue permissions (if using AWS Glue as the data catalog)::
...
S3 permissions (for both the data source bucket and the S3 results bucket)::
...
> * xref:connections-amazon-athena-deactivate.adoc[]
