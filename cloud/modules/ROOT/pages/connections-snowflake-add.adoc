...
:jira: SCAL-91104, SCAL-196178, SCAL-136317 (multiple confiurations for snowflake), SCAL-248181, SCAL-247865, SCAL-262244, SCAL-270847, SCAL-263432, SCAL-320916

[.lead]
Learn how to connect ThoughtSpot Cloud directly to a {connection} data source using supported authentication methods.
...
NOTE: Your key must be in *PKCS#8* format, and you must create it using the OpenSSL library provided in link:https://docs.snowflake.com/en/user-guide/key-pair-auth[Snowflake documentation].
...
[#x509]
=== OAuth Client Credentials with x509

For OAuth authentication with client credentials and x509, do the following:

. Select *OAuth Client Credentials with x509*.
. Enter Account name or your {connection} URL, OAuth Client ID, Private Key, Passphrase (optional), Certificate, Access token URL, Scope, Role (optional), and Warehouse (optional).
...
> * xref:connections-snowflake-deactivate.adoc[]
