...
:jira: SCAL-190426, SCAL-264258, SCAL-300461
...
[#inter-org-limitation]
== Limitation: importing Monitor alert TML across orgs

[IMPORTANT]
====
Importing a Monitor alert TML file into a *different org* fails when the alert is associated with a Personalized View (PV). The Alert TML stores static references to the source org's Liveboard ID (`pinboard_id`), visualization ID (`viz_id`), and Personalized View ID (`personalised_view_id`). When a Liveboard TML is imported into a new org, ThoughtSpot generates *new* IDs for the Liveboard and its visualizations. The Alert TML is not automatically updated to reflect these new IDs, so it continues to reference the original org's IDs, which are invalid in the destination org.

*Workaround — manually update IDs before or after import:*

. In the destination org, locate the newly created Liveboard and note its new *Liveboard ID* (`pinboard_id`) and *visualization ID* (`viz_id`).
. If the alert uses a Personalized View, locate the correct *Personalized View ID* (`personalised_view_id`) in the destination org.
. Open the Alert TML file and replace the old `pinboard_id`, `viz_id`, and `personalised_view_id` values with the new destination org IDs.
. Save and re-import (or re-publish) the updated TML.
====
...
+
IMPORTANT: This ID is org-specific. If you import this Alert TML into a different org, you must replace this value with the Personalized View ID from the destination org before or after import. See <<inter-org-limitation,Limitation: importing Monitor alert TML across orgs>>.
...
+
IMPORTANT: This ID is org-specific. When a Liveboard TML is imported into a different org, ThoughtSpot generates a new Liveboard ID. The Alert TML is not automatically updated to reflect the new ID. If you are migrating an alert across orgs, you must manually update this value to the new Liveboard ID in the destination org. See <<inter-org-limitation,Limitation: importing Monitor alert TML across orgs>>.
...
+
IMPORTANT: This ID is org-specific. When a Liveboard TML is imported into a different org, ThoughtSpot generates new visualization IDs. The Alert TML is not automatically updated to reflect these new IDs. If you are migrating an alert across orgs, you must manually update this value to the new visualization ID in the destination org. See <<inter-org-limitation,Limitation: importing Monitor alert TML across orgs>>.
