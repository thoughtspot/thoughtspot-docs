...
TS: Query Stats::
Contains data related to billable query consumption for customers on the query-based xref:consumption-pricing.adoc[consumption pricing] model. ThoughtSpot now distinguishes between regular query consumption and Spotter queries. Used for the xref:query-stats.adoc[Billable Query Stats Liveboard].
