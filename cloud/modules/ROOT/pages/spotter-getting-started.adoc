...
:description:
:jira: SCAL-228500, SCAL-244132, SCAL-239699, SCAL-227908, SCAL-230530, SCAL-220576
...
NOTE: Values only appear if indexing is enabled. If indexing is not enabled, you will see only column names and select ThoughtSpot keywords, weekdays, and system variables.

. Enable this feature using the URL flag, by adding the following to your ThoughtSpot URL:
+
*?enableSpotterShortcuts=true#*
. Type @ to enter lookup mode.
. Select from the available columns, filter values, and search data keywords to complete you search.
****
...
. Click on a Spotter token that shows a warning, identifiable by the exclamation point icon and yellow border.
...
Admin users can decide which LLM to use to enable all ThoughtSpot AI features. By default, ThoughtSpot uses Azure OpenAI. We also support Google Gemini and Snowflake Mistral.
