...
:jira: SCAL-267909, SCAL-300551, SCAL-306619, SCAL-308758, SCAL-308946, SCAL-320863
...
Data model instructions are for hard constraints that must always apply to every query on this data model — cases where you need strict, verbatim enforcement rather than the contextual application memory provides. For most business logic (evolving definitions, query patterns, metric formulas), use memory from Liveboards or conversations instead.
...
- Requires data model edit access or Can manage Spotter privilege (Can manage Spotter holders can manage data model instructions on any data model they can read)
- Applies to all users querying the Model
- Up to 10,000 characters supported
- Note: data model instructions are separate from Spotter instructions (which apply to Spotter broadly across the Org, not per Model)
...
These instructions are globally applicable and influence all queries made by any user on the data model, unlike other methods that also have a user-level scope. ThoughtSpot supports up to 10,000 characters of instructions.
...
Users with the *Can manage Spotter* privilege can also manage data model instructions on any data model they have read access to. (Data model instructions are Model-level; they are separate from Spotter instructions, which apply to Spotter more broadly.) This does not change who can edit the data model itself.
...
Not recommended for mathematical formulas:: Data model instructions are not well-suited for teaching the system mathematical formulas or complex calculations. For these scenarios, we recommend other methods, such as providing a reference question with xref:spotter-context.adoc[natural language context].

Avoid conflicting memory sources:: The instructions you provide must be coherent with each other and with reference questions and business terms.  Before adding a new instruction, review your existing instructions, reference questions, and business terms to ensure they tell a consistent story. If two memory assets define the same concept differently, the system cannot reliably choose between them and results will be unpredictable.
