...
If the join condition is more complex than the UI supports, you don't have to go to the TML editor. Instead, click the *Edit in TML* link in the _Create Join_ window, and then add your condition(s) in TML format.

.Tips for working with complex conditions
[TIP]
====
* Any complex condition which has been already created in the TML is supported.
* For any condition not supported by the UI, the join condition TML is displayed directly in the _Create Join_ window, so you know the join condition(s) whether they were created in the UI or in TML.
====
