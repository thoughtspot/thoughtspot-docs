...
By default, pivot table summaries are calculated as separate SQL queries. The pivot total is the aggregate as defined for the column.  For example, assuming that the formulas is `count(customerID)`, the total will be a count. It is not possible to customize the total’s aggregate. For example, modify the total to be an average where the line calculation is a count.
