...
The following example shows how the table joins were created in the Sales table of the Retail Sales Model, available in your *try.thoughtspot.com* account created through {connection} Partner Connect.
...
You can easily search the joined tables, without having to create a Model.
...
A Model is a curated dataset built for ad hoc analysis, that allows you to translate data from a database into the language of your business users.

Examples of things you can do in a Model include:

* Removing columns that aren't needed
* Adding data labels and synonyms
* Adding calculations, such as margin

The Model based on the Sales table on *try.thoughtspot.com* was created by doing the following:

. Select *Data workspace*.
. Click the *+ Create new* button, and select *Model*.
//. Select the plus icon, next to Sources.
//+
//image:worksheet_add_sources_link.png[Click the plus icon next to Sources]
. On the *How do you want to model your data* page, select a Dataset.
...
. Double-click each column from the Customers table that you want to include in the Model.
+
...
. Use the same process to select columns from the other tables to include in the Model.
+
 ...
 ** Store_County
+
image::partner-connect-columns-worksheet.png[Model in progress with the columns specified earlier in the article]

+
NOTE: As a best practice, you would not select a key from a table when creating a Model, because you would not want to search for the key.

. Select the pencil icon image:icon-edit-10px.png[edit icon] next to the current name of your Model, enter the name *Retail Sales*, and select *Done*.
+
image::partner-connect-worksheet-title.png[Edit Model name]
. Click the *more options* icon image:icon-more-10px.png[more options menu icon], and select *Save*.
+
image::partner-connect-worksheet-save.png[Save the Model]
+
Now, let's add a percent gross margin formula to the Model.
...
. Save the Model with the formula added, by selecting the *more options* icon image:icon-more-10px.png[more options menu icon], and selecting *Save*.
. Select *Data*, and select the Retail Sales Model.
...
. Save the Model with the updated formula settings, by selecting the *more options* icon image:icon-more-10px.png[more options menu icon], and choosing *Save*.

=== Best practices for Models

The best practices for data modeling also apply to Models.
...
+
image::partner-connect-worksheet-best.png[Model the data in your Model]

=== Adding a currency and geo map to a Model

To further enhance the usability of a Model, you can add a specific currency type to monetary values, and a geographic map to regions in your data.

Using the Retail Sales Model example, here's how geo maps and currency could be added:

. Select *Data*, and choose the *Retail Sales* Model.
...
. Select *Save Changes*.
+
Now that both currency and geographic types are set, you can see those changes reflected when you search the Retail Sales Model.

. Select *Search*.
. Select *Choose sources*.
. Deselect any tables previously selected (if needed), select only the *Retail Sales* Model, and select *Close*.
