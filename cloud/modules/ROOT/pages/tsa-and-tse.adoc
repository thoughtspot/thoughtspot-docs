= ThoughtSpot Analytics and ThoughtSpot Embedded defined
...
*TSE* is designed for external use, enabling customer analysts and developers to create and manage analytics content in their ThoughtSpot cluster for delivery in an external application. All analytics usage should occur through the external application through embedded sessions. TSE customers can use the capabilities of TSA for the sole purpose of authoring, distributing, and reporting on the usage of the ThoughtSpot embedded use case(s).
...
|Definition
|ThoughtSpot is used for our customers' own internal business and can be accessed directly.
|ThoughtSpot is used for our customers' own internal business and accessed via an internal portal by employees.
|ThoughtSpot Embedded refers to the standard OEM definition in which ThoughtSpot is used as a component in our customers’ solution (product, portal, etc.) which is then offered to their own customers.
...
|Examples
|Customer uses ThoughtSpot to enable their C-Suite to get analytics on the company's operational efficiency.
|Customer embeds ThoughtSpot in their Salesforce portal for internal use by their sales team.
|Customer embeds TS into their solution to provide an AI and analytics option in their offering.
|===
...
[cols="3h,3,3,3"]
|===
.^|*Use cases \ TS Edition* ^.^|*TSA* ^.^|*TSA with Embedded Analytics Add-On* ^.^|*TSE*
...
|Can only log in using basic authentication.

Cannot log in a ThoughtSpot user per session and use RLS / object access control using users / groups in ThoughtSpot)
^.^|image:checkmark_black.svg[Yes,15]
^.^|image:checkmark_black.svg[Yes,15]
|===
