...
:linkattrs:
:experimental:
:jira: SCAL-293512, SCAL-310400, SCAL-317812
...
== Create a Spotter Analyst using Analyst Builder
...
Users with Spotter privileges can create Spotter Analysts easily using Analyst Builder. In this chat-based interface, you can describe the analyst you would like to use, and Spotter translates your requests into instructions for the agent. To create a Spotter Analyst:
...
. The *Create Analyst* page appears. The right side shows the traditional fields to enter your choices and instructions manually, while the left side shows a chat interface you can use to request Spotter build an Analyst for you.
+
[.bordered]
image::spotter-analyst-builder.png[Spotter analyst builder]


. Enter your prompt in the chat interface. For example, you could say: `I want an Analyst that can help me understand sales performance`. Click the blue arrow or press Enter to start your conversation. Analyst Builder can ask you clarifying questions or interview you to form the Analyst you need.
+
NOTE: Be expressive in what you want, and describe what kinds of things you would like the Analyst to do for you, so the Analyst Builder can enunciate it before writing the instructions or identifying the right connectors or choosing the data model.

. The Spotter Analyst Builder searches for Models and Connectors that fit your prompt, and begins to draft instructions for your Analyst. It enters its draft into the interface on the right side, where you can edit it as needed. Note that Spotter Analyst Builder can only add Models and Connectors that you have access to.

. Click *Save* to create your Analyst. The *Share Analyst?* pop-up appears, prompting you to select which users or groups you want to have access to the Analyst.

Note that Analyst instructions override the agent-level instructions set by your Org.


== Create a Spotter Analyst manually

To create a Spotter Analyst manually:
...
. The *Create Analyst* page appears. The right side shows the traditional fields to enter your choices and instructions manually, while the left side shows a chat interface you can use to request Spotter build an Analyst for you.
+
Use the right side interface to name your Analyst, write the description that appears when you open it in Spotter, give it instructions on how to answer your questions, and specify which data source(s) and connectors it can access. You can click the camera icon to upload in image to serve as the Analyst icon. You can select up to five data models for your Analyst. Note that for instructions, you are limited to 5000 characters.
...
Don't — those are metric definitions. Define them once in the Model and every question inherits them. The Marketing analyst's instructions then only carry the Analyst-level part: "compare channels on CPL, normalize before comparing spend, lead with the metric then the trend, flag outliers."
...
>
> * We highly recommend that you register for the free Spotter analysts course in ThoughtSpot University that covers the information in this article, and watch the https://training.thoughtspot.com/spotter-analysts/2554906[Introduction to Spotter analysts^] lesson.
