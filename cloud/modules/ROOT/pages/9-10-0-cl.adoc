...
ThoughtSpot Sync from unsaved Answers [.badge.badge-ga-relnotes]#GA#:: You can now create syncs from Answers without saving first.
...
[#contextual-alert]
Contextual alert creation on KPI charts [.badge.badge-ga-relnotes]#GA#:: ThoughtSpot now preserves the filters you use on a KPI when you create a Monitor alert. Users with view access to the underlying Liveboard can now receive alert emails for filtered KPIs without saving the filters on the Liveboard. For more information, see xref:monitor.adoc[Monitor alerts].
...
[#commenting-on]
Commenting on Liveboards [.badge.badge-ga-relnotes]#GA#:: You can now add or reply to comments, tag other users, or subscribe to comment threads directly on a Liveboard. Users can point out insights or reach out for clarification without leaving ThoughtSpot. To try it out, click the comment icon to the right of the edit icon, and select the data point you would like to comment on. For more information.
...
//For more information, see xref:chart-settings-advanced.adoc[Advanced chart settings].

// Naomi -- scal-141936. actually beta. spotiq-change.adoc#iterative
[#run-change]
Run change analysis iteratively [.badge.badge-ga-relnotes]#GA#:: You can now drill down on specific columns in change analysis, in order to identify change drivers within your data. When you right-click on an attribute value in the change analysis window to drill down, ThoughtSpot adds the attribute value as a filter and re-calculates change analysis for the specific window of time specified in your original search. For more information, see xref:spotiq-change.adoc#iterative[Iterative change analysis].
...
[#natural-language]
Natural language search chart type [.badge.badge-ga-relnotes]#GA#:: You can now specify the answering chart type when you use Natural Language Search. For example, search for "sales by region pie chart", and the results appear in pie chart format.
//For more information, see xref:ai-answers.adoc[].
...
[#roles-and]
Roles and more granular access privileges [.badge.badge-ga-relnotes]#GA#:: ThoughtSpot introduces role-based access control (RBAC) to allow for more granular access privileges. Use roles to apply privileges customized for your organizational needs.
...
[#mobile-enhancements]
Mobile enhancements [.badge.badge-ga-relnotes]#GA#:: We made the following improvements to ThoughtSpot mobile:
...
[#google-cloud]
Google Cloud SQL for MySQL connection [.badge.badge-ga-relnotes]#GA#:: We added support for xref:connections-google-cloud-sql-mysql.adoc[Google Cloud SQL for MySQL].
...
// Mark -- SCAL-161198
[#looker-modeler]
Looker Modeler connection [.badge.badge-ga-relnotes]#GA#:: You can now create a connection to Looker Modeler. For more information, see xref:connections-looker.adoc[Looker Modeler].


// Naomi -- scal-169065, documentation JIRA scal-201046
[#dbt-public]
dbt public API [.badge.badge-ga-relnotes]#GA#:: dbt core users can now use the dbt public API to upload dbt project files to ThoughtSpot. Use the dbt sync API to upload your dbt project's zip files.

// Naomi -- scal-169614, documentation JIRA scal-201047
[#dbt-version]
dbt version 1.7 [.badge.badge-ga-relnotes]#GA#:: ThoughtSpot now supports dbt version 1.7.
