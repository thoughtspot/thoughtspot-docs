...
:page-last_updated: 9/4/2025
:experimental:
:linkattrs:
:page-aliases:
:description: ThoughtSpot Cloud 10.12.0.cl release notes

These new and enhanced features of ThoughtSpot Cloud were released in September 2025.

== ThoughtSpot Cloud 10.12.0.cl

=== Features removed in 10.12.0.cl
...
[#spotter-coach]
Spotter Coach business terms [.badge.badge-ga-relnotes]#GA#::
...
[#coach-spotter]
Coach Spotter from conversation [.badge.badge-early-access-relnotes]#Early Access#:: You can now coach Spotter without leaving your conversation. Click *Add to coaching* below an Answer to teach Spotter how to answer similar questions in the future. To enable this feature, contact your administrator. For more information, see xref:spotter-reference-questions.adoc[].

// Naomi. jira: SCAL-249300 docs jira: SCAL-267381, SCAL-267909
[#spotter-natural]
Spotter data model instructions [.badge.badge-beta-relnotes]#Beta#:: You can now use data model instructions to provide global feedback to Spotter that carries over to any query on the underlying data model. To provide data model instructions, select the instructions icon in Spotter and type a directive about how Spotter is to interpret the question. For example, you can instruct Spotter to specify the date filter as the last 30 days if the user does not provide a date keyword. To enable this feature, contact {support-url}.
+
For more information, see xref:data-model-instructions.adoc[].
...
[#rerun-queries]
Rerun queries in Search data [.badge.badge-ga-relnotes]#GA#::
...
[#kpi-alert]
KPI alert emails [.badge.badge-ga-relnotes]#GA#:: KPI alert emails now display a visualization of your KPI, along with the change from the previous time bucket, and the threshold condition.

// Naomi. Jira: SCAL-246787. Docs jira: SCAL-267138
[#query-groups]
Query_groups optional grouping columns [.badge.badge-ga-relnotes]#GA#:: Optional grouping columns are now available to all users and on by default. ThoughtSpot supports optional grouping columns in query_groups to ensure that only specified columns are included, if they are present in the query. A new syntax allows users to define an explicit optional list of grouping columns. Previously, you needed to manually exclude all other columns from the Model.
+
...
[#last-value]
Last value in period and first value in period functions [.badge.badge-ga-relnotes]#GA#::
...
[#bridge-support]
Bridge support for cloud data warehouses [.badge.badge-ga-relnotes]#GA#::
...
Multiple configurations per connection [.badge.badge-ga-relnotes]#GA#:: In addition to xref:connections-snowflake-add.adoc#additional-configurations-create[Snowflake], we now support multiple configurations for xref:connections-databricks-add.adoc#additional-configurations-create[Databricks] and xref:connections-gbq-add.adoc#additional[Google BigQuery]. This feature allows you to allocate separate configurations for different ThoughtSpot users, groups, or processes, eliminating the need to duplicate Liveboards and configure multiple connections, and helping you with cost tracking and governance. You can also use this for your system processes so that you can control and balance the computing load.
