...
:jira: SCAL-172330, SCAL-201842 (doc enhancement), SCAL-176032, SCAL-229340, SCAL-230176, SCAL-235033, SCAL-240707, SCAL-261112, SCAL-274760, SCAL-274862, SCAL-198739, SCAL-312775
...
To learn how to create a custom calendar from an existing date table, see xref:connections-cust-cal-create.adoc#upload[Upload a file for a custom calendar].
...
=== #Spotter and custom calendar awareness#

Spotter 3 and Spotter Agent are aware of your custom calendar configuration. When you ask date-based questions in Spotter, it resolves date expressions against your custom calendar table rather than applying Gregorian calendar defaults.

This means queries such as "show me sales last quarter" or "compare the last 28 days to the previous 28 days" return results based on the period boundaries defined in your custom calendar.

For examples of date and calendar-aware questions you can ask in Spotter, see xref:spotter-capabilities.adoc#date-calendar-queries[Date and calendar-aware queries].
...
* You can now add row-level security rules to custom calendars. To enable this feature, contact {support-url}. For more information, see xref:connections-cust-cal-create.adoc#row-level[Set up row-level security in a custom calendar]. Note that we now support multi-tenant calendars.
...
+
CAUTION: Custom calendar does not support a filter such as `order date >= august 2025`. An alternative search could be `order date on or after 31/08/2024` or `order date after 12 months ago`.
