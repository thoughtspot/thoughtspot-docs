...
:page-last_updated: Jul 28, 2026
:author: Naomi
:linkattrs:
:experimental:
:description: You can now migrate memory sources between Models or Worksheets using the legacy memory source TML file.
:jira: SCAL-226681, SCAL-253054, SCAL-254633, SCAL-250703, SCAL-265156, SCAL-320863, SCAL-331061
:page-aliases: migrate-coaching.adoc
:jira: SCAL-339595
...
For more information about TML files, see xref:tml.adoc[].

NOTE: **Data model instructions** use a separate migration path — see xref:spotter-migrate-instructions.adoc[Migrate instructions].  Memory generated from Liveboards and conversations uses the memory migration API — see xref:spotter-migrate-memory.adoc[Migrate memory].
...
Across different instances:: A common scenario where analysts test Spotter on a Model in a development environment and, once satisfied with the results, replicate the memory sources to the production environment for business users. This applies to both Cloud and Embedded customers.
Within the same instance or environment:: Users often migrate memory sources from one Model to another within the same instance.
+
Use Cases:
+
--
...
Across different Orgs:: Embedded customers often operate multiple Orgs within an instance and need to migrate memory sources from one Model in a primary Org to Models in other Orgs.
...
From one source Model to multiple target Models:: Setting up multiple Models in one cluster in one session:
+
--
...
Migration ensures consistency and allows for seamless replication of the training without any adjustments.
...
You need to adapt the memory source entries to keep them relevant and effective after migration.
...
////
. If you select *Export memory TML*:
...
+
[.bordered]
image::export-feedback-tml.png[Export legacy memory source TML]
////
...
NOTE: No memory source file is created if there are no memory source entries associated with the Model.
...
To export reference questions and business terms as TML, use the Model view steps above or the API below. To migrate Liveboard and conversation memory, see xref:spotter-migrate-memory.adoc[Migrate memory].
...
+
NOTE: Ensure that the GUID in the legacy memory source TML matches the target Model GUID.
...
+
[.bordered]
image::feedback-import.png[Import memory sources]
...
+
[.bordered]
image::data-workspace-utilities_v2.png[Data workspace utilities]
...
| axis_config | Mandatory. | Optional.
...
+
NOTE: We do not currently support importing a new Model and its memory sources in the same zip file.

* Check the sanity of the TML file, making sure you don't have missing fields, invalid entries for fields like `type`, `access`, `chart_type`, or `display_mode`, invalid `search_tokens` for the Model, or invalid formula expressions for the Model that memory source entries are being targeted for.

* Check for similar entries within the file: No two memory source entries can have the same `feedback_phrase`. This can occur when uploading memory source entries on the user level, which can have the same phrase. The system will throw an error when uploading multiple such entries. You have the option to edit the memory source TML to avoid such scenarios.
...
> **Migrating everything at once?** This page covers reference questions and business terms only.
> - For Liveboard and conversation memory → xref:spotter-manage-memory.adoc[Migrate memory] REST API v2, available since 26.8.0.cl. Personal memory is not included.
> - For Spotter instructions and data model instructions → xref:spotter-migrate-instructions.adoc[Migrate instructions]
