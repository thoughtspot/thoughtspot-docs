...
:description:
:jira: SCAL-228500, SCAL-244132, SCAL-253102, SCAL-258782, SCAL-265156, SCAL-264258, SCAL-267209, SCAL-291083
...
Coaching should be applied only when Spotter signals that it notices ambiguity after the data model has been optimized.

For detailed guidance on prerequisites and the recommended order of using coaching tools, see xref:spotter-coach-not-coach.adoc[].
...
Reference questions help Spotter learn patterns of intent and logic, expecially when paired with clear xref:spotter-ai-context.adoc[natural language context].
...
* Data model instructions are global and apply to all queries on the data model.
* Reference questions and business terms are specific to the data model but can be applied specifically at the user level (that is, only the user who created the coaching will have it apply to their searches).
* Reference questions can be added from a conversation. Business terms can be added directly from the Data workspace.
...
. Add coaching only where Spotter gets confused.
. Start with global rules, then add examples and definitions as needed.
...
ThoughtSpot Spotter</h5>
    <p class="card-text">Comprehensive coaching guide with everything from planning, testing, launching, and continuously improving coaching.</p>
