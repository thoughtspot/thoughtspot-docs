...
| Date or time | `DATE`,  `DATE_TIME`, `TIMESTAMP`, `TIME` | DATE_TIME, TIMESTAMP, and TIME are stored at the granularity of seconds. TIMESTAMP is identical to DATE_TIME, but is included for syntax compatibility.
...
After loading the data, ThoughtSpot sets the geo config data under auto select. You can also designate it as a geographical data type when you xref:data-modeling-edit.adoc#[edit the system-wide data model].
