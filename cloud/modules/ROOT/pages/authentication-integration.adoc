...
If your cluster uses per-org IdP configuration with IAMv2, ThoughtSpot automatically enforces Org isolation on incoming SAML group and Org claims. For more information, see xref:saml-okta.adoc#per-org-saml-isolation[Org isolation for per-org SAML authentication].
====
====
...
+
If your organization uses xref:orgs-overview.adoc[multi-tenancy with Orgs], ensure that you are in the Org you want to configure, in the *All orgs* section of the Admin Console.
