...
[#date-picker]
Date picker enhancement in Search [.badge.badge-ga-relnotes]#GA#:: We added support for rolling, fixed, and custom date filters when creating an Answer or filtering a Liveboard. Previously, when adding a date filter to a Search, ThoughtSpot supported the conditions `ON (=)`, `ON OR AFTER (>=)`, `BEFORE (<)`, and `BETWEEN`. Now, you can additionally filter for `ON OR BEFORE (\<=)`, `NOT BETWEEN`, `ON LAST`, and `ON NEXT`. To try it out, click the filter icon next to the Date column in the left side menu, or click the date filter below the Answer or Liveboard title. For more information, see
xref:date-filter.adoc[Date filters for Answers and Liveboards].
[#monitor-alerts]
Monitor alerts in ThoughtSpot notifications [.badge.badge-ga-relnotes]#GA#:: You can now see notifications for threshold and anomaly alerts in the ThoughtSpot notification service, rather than receiving emails.

[#single-value]
Single value selection for attribute filters [.badge.badge-ga-relnotes]#GA#:: When creating a Liveboard, you can now designate attribute filters as single-select, in addition to multi-select. For example, you could choose to make a Country filter single-select, so users would only see data related to one country at a time.

[#geomap-chart]
Geomap chart enhancements [.badge.badge-ga-relnotes]#GA#:: ThoughtSpot now supports the following enhancements for geomap visualizations:
...
Responsive Liveboards [.badge.badge-ga-relnotes]#GA#:: We have improved the responsiveness of Liveboards. For windows between 1024 pixels and 500 pixels, the Liveboard columns respond to re-sizing. For windows smaller than 500 pixels in width, a horizontal scroll bar appears.
[#chart-display]
Chart display enhancements [.badge.badge-ga-relnotes]#GA#:: When you create bar and column charts and slice by color, ThoughtSpot automatically enables gridlines to make the chart easier to read. You can choose to disable gridlines under the chart configuration menu.
...
[#natural-language]
Natural language search enhancements [.badge.badge-ga-relnotes]#GA#:: We made the following enhancements to natural language search:
...
[#pivot-table]
Pivot table enhancements [.badge.badge-ga-relnotes]#GA#:: We made the following enhancements to pivot table charts:
...
[#admin-controls]
Admin controls for new data panel experience [.badge.badge-ga-relnotes]#GA#::
...
[#headline-creation]
Headline creation and pinning deprecation [.badge.badge-ga-relnotes]#GA#:: ThoughtSpot is deprecating the pin Headline to Liveboard feature. Beginning in the 9.4.0.cl Cloud release, users will no longer see the pin icon when they select table summaries. To visualize and pin single aggregated measures, attributes in string/date format, or time-series metrics with sparklines, we recommend creating xref:chart-kpi.adoc[KPI charts]. Existing Headlines pinned to Liveboards will not be affected in this release. For more information, see
...
[#introduction-of]
Introduction of roles and more granular access privileges [.badge.badge-ga-relnotes]#GA#::
...
[#redshift-oauth]
Redshift OAuth with Azure AD IDP [.badge.badge-ga-relnotes]#GA#:: Redshift now supports external OAuth through Microsoft Azure AD. See xref:connections-redshift-azure-ad-oauth.adoc[Configure Azure AD external OAuth for a Redshift connection].
