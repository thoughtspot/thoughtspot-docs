...
:jira: SCAL-177005, SCAL-212568, SCAL-228737, SCAL-243212, SCAL-256725, SCAL-264562, SCAL-276164, SCAL-272806, SCAL-301561, SCAL-304841, SCAL-312999
...
[#advanced]
==== Advanced KPI display options

Navigate to the Column Settings tab of a KPI chart and toggle to *Advanced* to:
...
[.bordered]
image::advanced-kpi-setting.png[See the advanced display options]

Navigate to the Display Settings tab of a KPI chart and toggle to *Advanced* to:

* Show changes as percentages or absolute values.
* Show interpretation of increases as positive or negative changes.
+
This designation helps Spotter report whether an increase in a measure is a positive or negative change when it creates a summary.
* Set different colors for anomalies, indicating whether they are above or below the expected value.

[.bordered]
image::kpi-display-settings.png[Display settings for a KPI]

==== KPI styling on Liveboards

For KPI charts without sparklines, you can now control font size on Liveboards. To edit KPI font size, follow these steps:

. Open the Liveboard containing the KPI. Select *Edit* from the *More* menu image:icon-more-10px.png[more icon].

. From the top bar, select *Styling*.

. Under *KPI settings*, select the desired font size from the dropdown. You can choose XS, S, M, L, or XL.
...
To access this feature, go to your KPI chart and click the *Analyze change* button at the bottom of the chart. You see a summary of all changes in the KPI. The *Run change analysis* pop-up allows you to customize the columns ThoughtSpot analyzes. Note that ThoughtSpot runs change analysis between the two most recent data points defined by the KPI’s time bucket (for example, `daily` or `weekly`).

For more information on comparative analysis in ThoughtSpot, see xref:spotiq-change.adoc[].

[#cca-single]
.Change analysis for single-point KPIs [.badge.badge-beta]#Beta#
****
You can now make your single-point KPIs more powerful and actionable with the new aggregated KPI chart. You can see how your KPI has changed from the previous period even for queries like 'sales last 14 days' or 'sales last 2 months'. Run change analysis to find the change drivers in your data. To enable this feature, contact {support-url}.

To run change analysis on a single-point KPI, follow these steps:

. Open the Liveboard containing your KPI, scroll to your KPI, and click *Analyze change*.
. ThoughtSpot runs change analysis comparing the current value of your KPI to the previous period defined by your filter (for example, `last 14 days`). You see the overview of all changes by the top five most relevant attributes, and you can click the attribute name on the sidebar to see a more detailed breakdown.

*Limitations*

* When this feature is enabled on a Liveboard that has multiple single-point KPIs and many Liveboard schedules created on it, then there is a chance of a memory spike. In case of a memory spike, contact {support-url} to disable this feature.

* You cannot set percentage-based alert conditions on single-point KPIs.
* We do not support custom comparison with this feature, the comparison is always with the immediate previous period of the same length.
* You cannot customize the window of comparison. For example, we do not support comparing the last ten days with five days before that.
* You cannot visualize a sparkline with this chart.
* We do not support iterative change analysis for single-point KPIs.
* Single-point KPI change analysis only supports columns with a `DATE` data type. The `DATE_TIME` data type is not supported for single-point KPI change analysis.
* We do not support the following date filters, as they do not indicate a specific period:
** Not in between
** Not on
** After
** On or after
...
You can also add your KPIs to the watchlist on your Home Page. The KPIs can be added to the watchlist from the,

* *Home Page:* Select *Add KPIs to your watchlist* and then select a single or multiple KPIs to add. For more information, see xref:thoughtspot-one-homepage.adoc#quick-links[Track important KPIs].
* *Liveboard:* Click the *more* menu image:icon-more-10px.png[more options menu icon] on the KPI visualization in the Liveboard, and select *Add to watchlist*.
* *Saved Answer:* Click the *more* menu image:icon-more-10px.png[more options menu icon] on the KPI in the saved Answer, and select *Add to watchlist*.
...
We do not support the "detailed" keyword for the date axis.
...
>
> * We highly recommend that you register for the free KPI Monitor Alerts course in ThoughtSpot University that covers the information in this article, and watch the https://training.thoughtspot.com/kpi-monitor-alerts/2218548[Create KPI Alerts^] lesson.
