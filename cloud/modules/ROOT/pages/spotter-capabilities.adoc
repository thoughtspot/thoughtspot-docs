...
:jira: SCAL-287506, SCAL-331175

////
.image:Spotter.svg[width=35,role=light-only]image:Spotter_Dark.svg[width=35,role=dark-only] Spotter 3 new capabilities
////

.image:Spotter.svg[width=35,role=light-only]Spotter 3 new capabilities
****
...
== Cross data model search


Cross data model search is a dynamic, multi-model-aware agent that identifies which data sources are needed to answer your query, searches across them, and combines the results into one synthesized response. If in doubt, Spotter asks you to validate the Model it has identified before querying.
...
[.bordered]
image::select-all-data.png[Select All data models in select data model modal]

For more information, see xref:spotter-cross-model.adoc[].
...
== #Date and calendar-aware queries#

Spotter understands your organization's calendar configuration and uses it to resolve date expressions accurately. You can ask date-based questions using natural language, and Spotter applies your org's actual fiscal or custom calendar — not a Gregorian default.

NOTE: Calendar-aware date resolution is available in Spotter 3 and Spotter Agent (Spotter 2). It requires ThoughtSpot Cloud 26.5.0.cl or later.

=== Standard and fiscal date expressions

Spotter resolves common relative date expressions using your org's calendar settings, including the fiscal year start month, week start day, and date format.
...
| Returns results for the most recently completed quarter according to your org's fiscal calendar — not the Gregorian calendar quarter.

| What was the revenue year to date?
| Calculates revenue from the start of your fiscal year to today, based on the fiscal year start date configured for your org.

| Show me the sales growth from Q1 to Q2 this fiscal year.
| Compares Q1 and Q2 as defined by your fiscal calendar, regardless of whether they align with the Gregorian calendar.

| How did we perform in FY 2025?
| Retrieves data for the full fiscal year 2025, using your org's fiscal year boundary — not the calendar year January–December.

|===

=== Custom calendar date expressions

If your data model includes columns associated with a custom calendar — for example, a 4-5-4 retail calendar — Spotter resolves date expressions directly against that custom calendar definition. This ensures that period boundaries and labels (such as week names and quarter labels) match what is defined in your custom calendar table.
...
| Show me total sales for this period.
| Returns data for the current period as defined in your custom calendar table, not a standard Gregorian period.

| What were the sales in Period 7 last year?
| Looks up the exact date boundaries for Period 7 in your custom calendar and filters results accordingly.

| Compare sales in the last 28 days to the previous 28 days.
| Uses the custom calendar's period definitions to apply the correct boundaries for 28-day rolling periods, avoiding Gregorian calendar offset errors.

|===

For information about setting up a custom calendar, see xref:connections-cust-cal.adoc[Custom calendar overview].
