...
In a Worksheet with multiple types of currency (for example, USD, GBP, etc.), if you use the *From a column* setup to specify the currency format, ThoughtSpot has the following limitations:

* The currency column is automatically selected in every query that uses its linked column, and you cannot remove it.

* In charts, the numbers in axes, data labels, and tooltips do not append the currency symbol. The currency symbol or ISO code will be appended in the table view.
