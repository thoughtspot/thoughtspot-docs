...
You can use any of the supported number formats for delimiters and number of digits to show using link:http://docs.oracle.com/javase/7/docs/api/java/text/DecimalFormat.html[Java Decimal Notation^].
...
****
ThoughtSpot now includes enhanced number formatting controls for measures that are marked as attributes in the data source. Set the number format to *Number*, *Currency*, or *Percentage* to ensure that measures marked as attributes are formatted with the appropriate number format.

. Click *Edit chart configuration*.
. Select a column to edit the column properties.
. Under *Number format*, select the appropriate number format from the *Category* drop down menu.
+
[bordered]
image:number-format.png[Number format menu]
...
You can change the date format used to display a column's values xref:model-data-ui.adoc[for a single table or Model] or, by editing the data model, for xref:data-modeling-edit.adoc[the entire ThoughtSpot instance].
To edit the data model file, you must have administrative privileges.
To change the date format, follow these steps:

. Decide if the change is for a specific table or Model, or for the entire instance.
. Find the *Format Pattern* for the column.
+
This is either a column in a single table or Model or a column in the data modeling file.
...
NOTE: ThoughtSpot now supports whole number formatting for Japanese Yen (JPY) currency. This feature is in beta and off by default. To enable it, contact {support-url}.
