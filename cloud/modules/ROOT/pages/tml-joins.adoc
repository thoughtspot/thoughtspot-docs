...
:object: joins
:jira: SCAL-257490, SCAL-264258

Joins do not exist as a separate type of TML file. Instead, they are present in Models, tables, views, and SQL views. To modify {object} using TML, download the objects that contain these {object} to flat files in `.TML` format, modify the files, and subsequently upload the files either to the same cluster, or to a different cluster.
