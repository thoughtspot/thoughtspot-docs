...
[#change-filter]
Change filter order for Answer filters [.badge.badge-ga-relnotes]#GA#::
...
[#attributes-in]
Attributes in pivot table cells [.badge.badge-ga-relnotes]#GA#:: You can now include attributes in the pivot table cells as well as measures. This is useful for cases when you want to summarize text data such as user role, pass/fail, or other attributes. For more information, see
xref:chart-pivot-table.adoc#attributes[Pivot table charts].
[#ability-to]
Ability to change legend position [.badge.badge-ga-relnotes]#GA#:: You can now configure the legend position of a chart, or hide the legend. The available options are left, right, above, and below the visualization. To try it out, open the chart configuration menu, select *Settings*, and choose between *Legend on right*, *Legend on left*, *Legend on top*, and *Legend on bottom*. You can select the Show legend toggle to hide or show the legend. See
xref:chart-change.adoc#legend[Changing charts].
[#liveboard-header]
Liveboard header visible when you scroll down [.badge.badge-ga-relnotes]#GA#::
...
[#optionally-change]
Optionally change contextual menu to left-click [.badge.badge-ga-relnotes]#GA#::
...
[#thoughtspot-cloud]
ThoughtSpot Cloud on Google Cloud [.badge.badge-ga-relnotes]#GA#:: ThoughtSpot Cloud will be available on Google Cloud Platform in addition to AWS. Customers can choose whether they want their SaaS deployed on AWS or GCP.
+
...
[#edit-a]
Edit a dbt integration [.badge.badge-ga-relnotes]#GA#::
...
[#amazon-aurora]
Amazon Aurora PostgreSQL connection [.badge.badge-ga-relnotes]#GA#::
...
[#amazon-relational]
Amazon Relational Database Service (RDS) PostgreSQL connection [.badge.badge-ga-relnotes]#GA#::
...
[#mysql]
MySQL connection [.badge.badge-ga-relnotes]#GA#::
You can now create connections from ThoughtSpot to MySQL. For details, see
xref:connections-mysql.adoc[MySQL].
[#specify-default]
Specify default connection for CSV uploads [.badge.badge-ga-relnotes]#GA#:: Previously, when multiple database connections were configured to receive CSV uploads without a specified table or worksheet for collocation, the files were randomly uploaded to one of the connections. With this new enhancement, administrators and data managers now have the ability to specify a default target connection for such uploads. +
