...
+
. Hover on the *Styling panel* button.
+
[.bordered]
...
== Limitations
* Note tiles cannot be configured via the styling pane.
* All the Answers and notes in a group fit in a single page when scheduling or downloading a PDF.
* All the Answers and notes in a group fit in a single slide when using the present mode.
* Grouping visualizations is not supported when the Liveboard is laid out with a grid of two or fewer columns. To group visualizations, there must be three or more present in one row. Grouping visualizations is also not supported if you adjust your browser's width to show only one or two columns.
